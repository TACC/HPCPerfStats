...
| `hpcperfstats/tests/test_sync_timedb_archive.py` (member cache) | DB-complete ingest path: identity-keyed `get_existing_archive_members_for_daily_archive` cache, `daily_archive_has_member_with_size`, Redis populate fallback + sealed point lookup (`test_sealed_archive_member_has_exact_size_early_exit`, `test_daily_archive_has_member_falls_back_when_populate_raises`, `test_raw_stats_path_needs_tar_append_conservative_on_redis_failure`), `invalidate_daily_archive_members_cache` (`test_daily_archive_members_cache_*`, `test_raw_stats_needs_append_uses_cache`, `test_single_member_early_exit_finds_match`, `test_dedupe_sealed_daily_archive_last_resort`, `test_get_existing_archive_members_uses_redis_l2`). |
| `hpcperfstats/tests/test_sync_timedb_archive_members_redis.py` | Redis L2 single-flight populate, incremental HASH + `complete` signal, waiter semantics, `dedupe_hint`, sealed-scan wiring + scan error re-raise (`test_populate_redis_members_from_sealed_scan_wires_stream_fn`, `test_stream_re_raises_archive_members_redis_unavailable_from_on_member`, `test_populate_scan_failed_includes_sealed_path`), hard-fail when Redis required (host `FakeRedis`; compose optional). |
