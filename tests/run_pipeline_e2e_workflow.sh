...
Prerequisites:
  docker-compose.yaml from this directory and tests/docker-compose.pipeline-e2e.yaml.
EOF
}
...
for _ in $(seq 1 60); do
  ...
  rmq_id="$($COMPOSE ps -q rabbitmq 2>/dev/null || true)"
...
$COMPOSE run --rm web python hpcperfstats/site/manage.py migrate --noinput
...
if [[ "$WITH_BROWSER" -eq 1 ]]; then
  ...
  web_up=0
  for _ in $(seq 1 60); do
    if $COMPOSE exec -T web python -c 'import urllib.request; urllib.request.urlopen("http://127.0.0.1:8000/machine/", timeout=3).read(8)' >/dev/null 2>&1; then
      web_up=1
      echo "web is up."
      break
    fi
