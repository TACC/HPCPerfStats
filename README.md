...
4. **Compose file:**
   ...
   **Startup snapshot wait:** on large trees with **`all`**, first pending rescan may wait up to **`sync_startup_snapshot_wait_seconds`** (default **300**, min **120**) for the janitor startup heavy pass to publish **`StartupArchiveScanCoordinator`** snapshot before single-flight fallback build. Grep **`sync_timedb: pending rescan begin`**, **`startup archive scan ready`**, **`janitor: discover_ready_day_close`**. Boot **`DAY_CLOSE`** discover runs on **`[sync_timedb:thread:archive-janitor]`** only — ingest is not gated on day-close completion. Inspect async manifest **`.sync_timedb_async_day_close.json`** for in-flight rows. These startup paths run only when the pipeline command includes **`all`** (see below).

   **Startup maintenance (`all` only):** janitor **`reason=startup`** heavy snapshot + **`DAY_CLOSE`** discover, handoff recover on the supervisor main thread, then **`startup maintenance idle; ingest may begin`**. Runs only when **`sync_timedb.py`** is invoked with **`all`** (production supervisord example: `sync_timedb.py all`). Date-window runs skip startup maintenance and begin ingest immediately.
   ...
   **Startup archive scan (single-flight):** on large trees with **`all`**, janitor startup maintenance publishes one **`build_archive_maintenance_snapshot`** via **`StartupArchiveScanCoordinator`** — tune **`sync_startup_snapshot_wait_seconds`** (default **300**, min **120**) if logs show long waits before **`startup archive scan ready`**. Details: **`docs/DEPLOY_CONCURRENCY_AND_NUMA.md`** § canonical startup archive scan.
