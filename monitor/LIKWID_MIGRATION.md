...
- **Multi-group PERF collect:** after DF/RAPL `perfmon_setupCounters` on later groups, each `host_cpu_hw` tick calls `likwid_pmc_adapter_prepare_collect()` → `perfmon_setupCounters(g_group)` then `readGroupCounters(g_group)`. Without re-setup, Turin (amd-rtx) can emit flat AMD canaries while DF/RAPL stay healthy. Judge AMD health on mid-row `retired_instructions` / `ls_dispatch` / `instr_retired` under load (not Intel FIXC/util).
