...
**Operator discovery validation is content-level** (not Read-only / shallow shape): `hpc_hook_lib.operator_discovery_issues` / `_validate_pending_commands_subsection` reject multi `#### <same-service>` blocks, host `cd` before `docker compose`, `--tail`/`--since` on `compose logs` before `grep`, unfiltered `compose logs` firehose, and heredoc `python3 - <<` through `exec`. Reading `compose-operator-terminal-commands.mdc` alone does **not** satisfy the gate — the live plan’s Pending commands must pass these checks. See unit tests in `hpcperfstats/tests/test_cursor_hooks.py`.
