...
static const struct intel_uncore_pci_cfg intel_bdw_qpi_pci_cfg = {
    .pci_dids = dids,
    .nr_pci_dids = 3,
    .events = events,
...
{
  return intel_uncore_pci_begin(&intel_bdw_qpi_pci_cfg, type);
}

static void intel_bdw_qpi_collect(struct stats_type *type)
{
  intel_uncore_pci_collect(&intel_bdw_qpi_pci_cfg, type);
}
