...
| **`sync_ingest_pool_maxtasksperchild`** (default **1**) | **Ingest pool only.** Recycle ingest worker after **every** file — drops idle pandas/Django heap (primary fix when one multi‑GiB file remains). With **`0`**, use expanded in-worker release plus supervisor cooperative recycle (**`sync_ingest_recycle_worker_on_failure`**, **`sync_ingest_cooperative_recycle_after_giant`**, fair-share **`sync_ingest_cooperative_recycle_rss_fraction`**); soak with **`sync_ingest_worker_memory_telemetry=yes`**. **Archive** and **sealed-archive CLI** spawn pools always use hardcoded **`maxtasksperchild=1`** (not this INI). **Stability mitigation (2026-07-09):** if idle-pool stalls persist at **`pool_recover terminate begin`** under fast `db_skip` catch-up with **`maxtasksperchild=0`**, set **`sync_ingest_pool_maxtasksperchild=1`** on the next redeploy — disables supervisor SIGTERM retire (`should_supervisor_retire_worker` no-op) and uses stdlib per-task recycle instead (higher RSS, fewer taskqueue-death stalls). |
...
**Troubleshooting — idle-pool ghost in-flight (multi-hour hang at fixed `remaining`):** Symptom: **`remaining` stuck** (e.g. **102**), no **`loading time`**, ingest workers idle (`futex_wait`/`pipe_read`), main supervisor in **`imap_sliding_window_watch_pool`** → **`time.sleep(poll_timeout_s)`**. Workers show **`pool.worker` → `queues.get()`** (idle, not parsing). **Not** the same as giant-tail supplement (`supplement=yes` fast completions while one giant parses). **Mitigation (code):** reconcile-first — **`INFO: pool imap idle reconcile redispatch round=…`** collects via **`get(timeout=0)`** or redispatches. When a reconcile round redispatches **all** pending paths (`redispatched_n == pending_async_n`) and workers stay idle, treat as **full-redispatch thrash** (taskqueue dead after cooperative SIGTERM retire / Pool IPC drift under **`sync_ingest_pool_maxtasksperchild=0`**) and run **one** **`INFO: pool imap idle reconcile pool_recover`** — dedupe pending normpaths (**`duplicate_pending_n`** / **`duplicate_sample`** in **`pool_recover skip_probe begin`**), skip idempotent DB-complete paths (summary in **`pool_recover skip_probe done`**), tear down + recreate the ingest Pool (**`pool_recover terminate elapsed_s`**), re-`apply_async` unique remainder (**`pool_recover resubmit n=`**). Sliding-window dispatch suppresses duplicate normpaths in **`pending_async`** (**`WARN: pool imap duplicate dispatch suppressed`**). Exit **124** only after reconcile rounds exhausted **and** recover absent/failed (`sync_pool_idle_reconcile_max_rounds`, default **3**); fatal lines then set **`likely_cause=idle_pool_taskqueue_dead`** (not `unknown`). Distinguish from exit **137** recycle gate (**`likely_cause=recycle_stuck`**). **`stats_rows=0`** on completed giants is often **duplicate-skip ingest** — do not use path-tail second alone for `host_data` probes (head timestamp differs). Missing **`worker_pid`** on giant_reap after `db_skip` logs **`WARN: … retire skipped missing worker_pid … likely_cause=meta_or_registry_gap`** only (never fatal). **Operator capture (before restart):** save main + worker py-spy to `/tmp` (`podman exec -u root … py-spy dump --pid $MAINPID`); disk-stat completed giants; grep **`ingest file path=`** / **`pool_recover`** / **`pool_recover done`** / **`idle_pool_taskqueue_dead`** for stuck paths. Then **`podman restart hpcperfstats_pipeline_1`** if recover does not resume.
