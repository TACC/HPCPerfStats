...
**Catch-up INI starting points** (until backlog of multi‑GiB segments clears): defaults above ship in **`hpcperfstats.ini.example`** with the release — **`sync_pool_process_cap=8`**, **`sync_ingest_pool_maxtasksperchild=1`**, **`sync_ingest_malloc_trim_after_file=yes`**, **`sync_process_tree_rss_limit_mb=96000`**. On **small dev compose** hosts (e.g. 8–16 GiB pipeline cgroup), set **`sync_process_tree_rss_limit_mb=0`** to disable tree defer and optionally raise **`sync_pool_process_cap`** if ingest is CPU-bound. Historical catch-up tuning: `sync_ingest_chunk_size=8–16`. On very large trees (~100k+ closed paths), optional interim disable until fixes ship: `sync_startup_day_close_preflight=no` (janitor **`DAY_CLOSE`** debt + tick discover remain). After startup-scan + janitor budget fixes, prefer leaving prefights **on** and tune `sync_startup_snapshot_wait_seconds` (default **300**), `archive_janitor_days_per_tick`, **`sync_day_close_max_inflight`** (default **2**). Set **`sync_startup_drain_day_close_before_ingest=no`** only when startup latency from day-close drain is unacceptable (steady-state ingest is not gated on cross-day raw deletion).

**Post-fix disk-release progress grep:** `janitor: discover_ready_day_close`, `janitor: day_close enqueue`, `janitor: day_close submit`, `Archive janitor tick done` (expect `debt_popped>0` or progressing day-close), `sync_timedb: startup archive scan ready`, `sync_timedb: pending rescan done`.
...
| **Bulk reclaim of sealed archived raw after restart** | Opt-in **`sync_startup_raw_removal_preflight=yes`** (default **`no`**) runs a background verify pass and gated delete manifest **`{archive_data_dir}/.sync_timedb_startup_raw_removal.json`**. The archive janitor remains the ongoing backstop. |
...
| **Quiescent dirty `.tar` at startup** | When a daily **`.tar`** is dirty but **no closed raw remains on disk** for that calendar day, **`StartupDayClosePreflight`** enqueues janitor **`DAY_CLOSE`** with **`reason=startup_quiescent_tar`**. Requires filesystem truth (`os.path.isfile` on every unprocessed path). |
| **Checkpoint-complete DAY_CLOSE at startup** | When all **mapped** closed raw for a calendar day is checkpoint-complete but **`.tar`** / raw remain on disk, **`StartupDayClosePreflight`** (default **`yes`**) enqueues via manifest shim → janitor **`DAY_CLOSE`** debt. Boot janitor tick also runs **`_discover_and_enqueue_ready_day_close`**. Steady-state cap: **`sync_day_close_max_inflight`** (default **2**); startup cap: **`sync_startup_day_close_max_inflight`**. Candidate log: **`queued` / `waiting_on_ingest` / `disqualified`**. |
