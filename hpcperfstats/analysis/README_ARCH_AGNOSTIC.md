...
- **Job metrics**: Node power estimate (`max_node_power_est_w`, `avg_node_power_est_w`) merges Intel/AMD RAPL (`intel_x86_rapl` / `amd_x86_rapl`, `pkg_energy`), Grace `host_cpu_hw` `dcgm_cpu_power_util_w` (sum of **unique** per-socket watt paints per host/time — not SUM across replicated CPU `dev` rows), and NVIDIA `power_usage` / `module_power_usage`.
