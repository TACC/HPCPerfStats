...
{
  return likwid_uncore_adapter_begin(type, LIKWID_UNCORE_PROFILE_R2PCI_IVB);
}

static void intel_ivb_r2pci_collect(struct stats_type *type)
{
  likwid_uncore_adapter_collect(type, LIKWID_UNCORE_PROFILE_R2PCI_IVB);
}

struct stats_type intel_ivb_r2pci_stats_type = {
  .st_name = "intel_x86_uncore_r2pci_ivb",
