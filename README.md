...
5. **Application config:**
   ...
   **Python interpreters (image):** web/gunicorn and helpers use GIL **`python:3.14.7-trixie`** (`/usr/local/bin/python3`). Pipeline daemons **`listend`**, **`sync_timedb`**, and **`update_metrics`** are baked onto free-threaded **`/opt/python3.14t/bin/python`** (no INI toggle). `docker compose exec pipeline python3` stays GIL for operator one-liners; greppable startup lines `python_abi executable=… Py_GIL_DISABLED=…` prove the live daemon ABI. Follow-on work may replace spawn/fork pools with in-process threads; Redis `job:v1` stays the durable orchestrator.
