...
| **pipeline** | `listend.py` | Pika consume on one BlockingConnection; **host-affine archive threads** (`listend_archive_worker_threads`, default **8**) for durable `current` append/rotate + threadsafe ack; drop-mode **`listend_amqp_prefetch`** (default **32**; pause keeps prefetch **1**). Optional live DB dual-write via host-affine spawn pool (`listend_db_ingest_*`, default **32** workers, **8 GiB** total queue budget, **`listend_db_ingest_batch_samples=100`**) with **POSIX SharedMemory** enqueue (`hps-ld-*`) + path+range fallback; compose **`pipeline.shm_size: "10gb"`**. Backpressure when live on: **`listend_db_ingest_backpressure`** default **`drop`** (archive+ack, shed live DB via `queue_drops`) or **`pause`** (high watermark **0.95** / any worker full → stop consume without archive until resume **0.50**; overflow on broker). | Low archive path (parallelized across hosts); DB slots when live ingest on — size with `sync_ingest_pool_processes` against Postgres `max_connections`. When live on, archive-backed catch-up remains completeness repair for live gaps; archive/delete needs a sync_timedb file-complete mark |
