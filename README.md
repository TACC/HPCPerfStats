...
4. **Compose file:**
   ...
   **Daily monitor archive compression:** `sync_timedb` seals each day’s `YYYY-MM-DD.tar` to **`YYYY-MM-DD.tar.zst`** with **zstd** (see `[PORTAL]` keys `archive_zstd_level`, `archive_zstd_threads`, `archive_zstd_long` in `hpcperfstats.ini.example`). Before raising `archive_zstd_level` above **9** on production data, benchmark a representative daily tar on the pipeline host: `zstd -b3 -e12 -T0 -S -- ./YYYY-MM-DD.tar`.
