...
| **Resources**                   | CPU or CPU+GPU watt-hours (when CPU power is available; title omits +GPU when the job has no GPUs), shared filesystem totals (`fsio`, including peak throughput and peak IOPS when computable), GPU summary (total/active GPUs and sum of per-GPU util % out of `gpu_count×100`), per-(host,dev) GPU inventory when device IDs exist (NVIDIA preferred, else AMD, else Intel PVC; legacy jobs without stored `dev` show a node-aggregate note), then client/server log links | Validate energy, I/O<sup>[11](#ref-11)</sup> volume and burst rates, compare GPU allocation vs activity and which devices worked, then jump to external logs. |
