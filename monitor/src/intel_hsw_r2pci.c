 ...
 *  Intel Haswell R2PCI uncore (intel_x86_uncore_r2pci_hsw).
 */
...
static const struct intel_uncore_pci_cfg intel_hsw_r2pci_pci_cfg = {
    .pci_dids = dids,
    .nr_pci_dids = 1,
    .events = events,
...
{
  return intel_uncore_pci_begin(&intel_hsw_r2pci_pci_cfg, type);
}

static void intel_hsw_r2pci_collect(struct stats_type *type)
{
  intel_uncore_pci_collect(&intel_hsw_r2pci_pci_cfg, type);
}
