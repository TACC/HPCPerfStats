 ...
 *
 * Applied once before dynamic import of @bokeh/bokehjs (see bokehInit.ts).
 */

const PATCH_FLAG = "__hpcperfstatsResizeObserverDeferred";

let nativeResizeObserver: typeof ResizeObserver | null = null;

export function scheduleBokehSafeResizeObserverCallback(
...
export function applyBokehResizeObserverDeferral() {
  ...
  function DeferredResizeObserver(callback: ResizeObserverCallback) {
  ...
  window.ResizeObserver = DeferredResizeObserver as unknown as typeof ResizeObserver;
}
