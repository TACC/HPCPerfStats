...
- **`compose-operator-terminal-commands.mdc`** — plans and blocked diagnostics: operator commands as one copy/paste `docker compose exec` block per service; findings recorded on disk per **`plan-live-disk-sync.mdc`**.
