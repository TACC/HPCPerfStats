...
int intel_uncore_pci_begin(const struct intel_uncore_pci_cfg *cfg, struct stats_type *type)
{
  ...
  if (pci_map_create(&dev_paths, &nr_devs, (int *)(uintptr_t)cfg->pci_dids, cfg->nr_pci_dids) < 0)
    TRACE("Failed to identify pci devices");

  for (i = 0; i < nr_devs; i++)
    if (intel_pmc_uncore_begin_dev(dev_paths[i], (uint32_t *)(uintptr_t)cfg->events,
                                   cfg->nr_events) == 0)
      nr++;
...
void intel_uncore_pci_collect(const struct intel_uncore_pci_cfg *cfg, struct stats_type *type)
{
  ...
  for (i = 0; i < nr_devs; i++)
    intel_pmc_uncore_collect_dev(type, dev_paths[i], cfg->event_keys, cfg->nr_events,
                                 cfg->fixed_ctr_key);
