...
from hpcperfstats.dbload.lib.sync_timedb_startup_day_close import PHASE_DISCOVERING, PHASE_DONE, StartupDayClosePreflight, manifest_path
...
def _make_preflight(tmp_path, async_coord, **kwargs):
    import zoneinfo
    defaults = {'archive_data_dir': str(tmp_path), 'host_name_ext': 'cluster.integration.test', 'tgz_archive_dir': str(tmp_path / 'daily'), 'local_tz': zoneinfo.ZoneInfo('UTC'), 'log_fn': MagicMock(), 'async_day_close': async_coord, 'get_disqualification_inputs': lambda: {'inflight_paths': set(), 'pending_append_by_daily_tar': {}, 'in_flight_archive_tars': set(), 'pending_archive_task_tars': set()}, 'get_unmapped_closed_raw_tars': lambda: set(), 'day_phases': lambda: {}}
    defaults.update(kwargs)
    async_coord = defaults['async_day_close']
    if not hasattr(async_coord, 'recover_stale_manifest_entries'):
        async_coord.recover_stale_manifest_entries = lambda: None
    preflight = StartupDayClosePreflight(**defaults)
    preflight.enabled = True
    return preflight

def test_startup_day_close_enqueues_async_for_checkpoint_complete_day(tmp_path, monkeypatch):
    day = date(2022, 7, 1)
    ...
    checkpoint_path.write_text(json.dumps([{'path': str(seg), 'size': seg.stat().st_size, 'mtime': int(seg.stat().st_mtime)}]))
    submitted = []
...
def test_startup_day_close_budget_applies_after_scans_not_before(tmp_path, monkeypatch):
    day = date(2022, 7, 2)
    seg = _make_closed_segment(tmp_path, 'cluster.integration.test', day)
    daily_dir = tmp_path / 'daily'
    daily_dir.mkdir()
    ...
    def slow_unprocessed(*_a, **_k):
        ...
        return {}
    monkeypatch.setattr('hpcperfstats.dbload.lib.sync_timedb_startup_day_close.build_unprocessed_raw_by_daily_tar', slow_unprocessed)
    monkeypatch.setattr(cfg, 'get_sync_startup_day_close_budget_seconds', lambda: 60.0)
    monkeypatch.setattr(cfg, 'get_sync_startup_day_close_days_per_slice', lambda: 5)
    monkeypatch.setattr(cfg, 'get_sync_day_close_candidate_report', lambda: False)
...
def test_startup_discover_backoff_skips_scan_when_async_saturated(tmp_path, monkeypatch):
    scan_calls = {'n': 0}
    ...
    class _SaturatedCoord:
        ...
        def recover_stale_manifest_entries(self):
            pass
    monkeypatch.setattr('hpcperfstats.dbload.lib.sync_timedb_startup_day_close.build_unprocessed_raw_by_daily_tar', counting_unprocessed)
...
def test_startup_discover_submits_up_to_startup_max_inflight_per_slice(tmp_path, monkeypatch):
    daily_dir = tmp_path / 'daily'
    daily_dir.mkdir()
    tars = []
    ...
    monkeypatch.setattr(cfg, 'get_sync_day_close_candidate_report', lambda: False)
    monkeypatch.setattr('hpcperfstats.dbload.lib.sync_timedb_startup_day_close.build_unprocessed_raw_by_daily_tar', lambda *_a, **_k: {})
    monkeypatch.setattr('hpcperfstats.dbload.lib.sync_timedb_startup_day_close.build_remaining_raw_stats_by_daily_gz', lambda *_a, **_k: {})
    monkeypatch.setattr('hpcperfstats.dbload.lib.sync_timedb_startup_day_close.days_ingest_complete_by_checkpoint', lambda *_a, **_k: list(tars))
    monkeypatch.setattr('hpcperfstats.dbload.lib.sync_timedb_startup_day_close.days_quiescent_tar_needs_day_close_at_startup', lambda *_a, **_k: [])
    preflight = _make_preflight(tmp_path, _FakeCoord())
...
def test_startup_day_close_uses_accrual_snapshot_when_present(tmp_path, monkeypatch):
    ...
    def boom_collect(*_a, **_k):
        collect_calls['n'] += 1
        return []
    monkeypatch.setattr('hpcperfstats.dbload.lib.sync_timedb_startup_day_close.build_unprocessed_raw_by_daily_tar', lambda *_a, maintenance_snapshot=None, **_k: collect_calls.update({'used_snapshot': maintenance_snapshot is not None}) or {})
    monkeypatch.setattr('hpcperfstats.dbload.lib.sync_timedb_startup_day_close.build_remaining_raw_stats_by_daily_gz', lambda *_a, maintenance_snapshot=None, **_k: {})
    monkeypatch.setattr(cfg, 'get_sync_day_close_candidate_report', lambda: False)
    snapshot = ArchiveMaintenanceSnapshot(closed_paths=[], mapping={})
    preflight = _make_preflight(tmp_path, MagicMock(get_disqualified_daily_tars=lambda: set(), submit_day_close=lambda *_a, **_k: False, active_or_submitted_tar_paths=lambda: set()), get_startup_snapshot=lambda: snapshot)
    preflight._discover_slice()
    assert collect_calls.get('used_snapshot') is True
    assert collect_calls['n'] == 0

def test_startup_day_close_shutdown_preserves_pending_eligible(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(cfg, 'get_sync_day_close_candidate_report', lambda: False)
    preflight = _make_preflight(tmp_path, _FakeCoord())
    preflight._manifest['pending_eligible'] = [os.path.normpath(str(tmp_path / 'daily' / '2022-07-11.tar'))]
    preflight._manifest['pending_retry'] = [os.path.normpath(str(tmp_path / 'daily' / '2022-07-10.tar'))]
...
def test_startup_day_close_multi_slice_submits_until_cap(tmp_path, monkeypatch):
    ...
    for d in days:
        ...
        checkpoint_entries.append({'path': str(seg), 'size': seg.stat().st_size, 'mtime': int(seg.stat().st_mtime)})
    ...
    original_eligible = 'hpcperfstats.dbload.lib.sync_timedb_startup_day_close.days_ingest_complete_by_checkpoint'
...
def test_startup_day_close_retries_after_disqualification_clears(tmp_path, monkeypatch):
    day = date(2022, 7, 30)
    seg = _make_closed_segment(tmp_path, 'cluster.integration.test', day)
    daily_dir = tmp_path / 'daily'
    daily_dir.mkdir()
    ...
    open(tar_path, 'wb').close()
    (tmp_path / '.sync_timedb_state.json').write_text(json.dumps([{'path': str(seg), 'size': seg.stat().st_size, 'mtime': int(seg.stat().st_mtime)}]))
    submitted = []
    disq = {tar_path}
...
def test_startup_day_close_disqualify_once_per_slice(tmp_path, monkeypatch):
    ...
    slice_logs = [str(call.args[0]) for call in log_fn.call_args_list if call.args and 'discover_slice:' in str(call.args[0])]
...
def test_startup_day_close_submits_quiescent_tar_with_stale_unprocessed(tmp_path, monkeypatch):
    day = date(2022, 8, 1)
    daily_dir = tmp_path / 'daily'
    daily_dir.mkdir()
    ...
    class _FakeCoord:
        ...
        def active_or_submitted_tar_paths(self):
            return {t for t, _ in submitted}
    monkeypatch.setattr('hpcperfstats.dbload.lib.sync_timedb_startup_day_close.build_unprocessed_raw_by_daily_tar', lambda *a, **k: {tar_path: [stale_path]})
    monkeypatch.setattr(cfg, 'get_sync_startup_day_close_budget_seconds', lambda: 60.0)
    monkeypatch.setattr(cfg, 'get_sync_startup_day_close_days_per_slice', lambda: 5)
    monkeypatch.setattr(cfg, 'get_sync_day_close_candidate_report', lambda: False)
    preflight = _make_preflight(tmp_path, _FakeCoord())
...
def test_startup_day_close_skips_quiescent_when_unprocessed_still_on_disk(tmp_path, monkeypatch):
    day = date(2022, 8, 2)
    daily_dir = tmp_path / 'daily'
    daily_dir.mkdir()
    tar_path = os.path.normpath(str(daily_dir / (day.isoformat() + '.tar')))
    ...
    class _FakeCoord:
        ...
        def active_or_submitted_tar_paths(self):
            return {t for t, _ in submitted}
    monkeypatch.setattr('hpcperfstats.dbload.lib.sync_timedb_startup_day_close.build_unprocessed_raw_by_daily_tar', lambda *a, **k: {tar_path: [str(on_disk_path)]})
    monkeypatch.setattr(cfg, 'get_sync_startup_day_close_budget_seconds', lambda: 60.0)
    monkeypatch.setattr(cfg, 'get_sync_startup_day_close_days_per_slice', lambda: 5)
    monkeypatch.setattr(cfg, 'get_sync_day_close_candidate_report', lambda: False)
    preflight = _make_preflight(tmp_path, _FakeCoord())
...
def test_discover_done_true_while_pending_eligible_only(tmp_path):
    preflight = _make_preflight(tmp_path, MagicMock(get_disqualified_daily_tars=lambda: set(), submit_day_close=lambda *_a, **_k: True, active_or_submitted_tar_paths=lambda: set(), is_complete=lambda _tar: False))
...
def test_boot_reconcile_resumes_when_phase_done_and_eligible_remain(tmp_path, monkeypatch):
    from hpcperfstats.dbload.lib.sync_timedb_startup_day_close import _save_manifest
    day = date(2026, 4, 22)
    seg = _make_closed_segment(tmp_path, 'cluster.integration.test', day)
    daily_dir = tmp_path / 'daily'
    daily_dir.mkdir()
...
def test_boot_reconcile_when_phase_done_and_async_still_incomplete(tmp_path, monkeypatch):
    daily_dir = tmp_path / 'daily'
    daily_dir.mkdir()
    tar_path = os.path.normpath(str(daily_dir / '2026-04-22.tar'))
    open(tar_path, 'wb').close()
...
def test_discover_loop_done_with_async_saturated_pending_eligible(tmp_path, monkeypatch):
    daily_dir = tmp_path / 'daily'
    daily_dir.mkdir()
    tar_paths = []
...
def test_boot_reconcile_false_when_only_pending_eligible_handoff(tmp_path, monkeypatch):
    ...
    class _FakeCoord:
        ...
        def is_complete(self, _tar_path):
            return True
    monkeypatch.setattr('hpcperfstats.dbload.lib.sync_timedb_startup_day_close.build_unprocessed_raw_by_daily_tar', lambda *_a, **_k: {})
    monkeypatch.setattr('hpcperfstats.dbload.lib.sync_timedb_startup_day_close.days_ingest_complete_by_checkpoint', lambda *_a, **_k: [])
    monkeypatch.setattr('hpcperfstats.dbload.lib.sync_timedb_startup_day_close.days_quiescent_tar_needs_day_close_at_startup', lambda *_a, **_k: [])
    preflight = _make_preflight(tmp_path, _FakeCoord())
    preflight._manifest['phase'] = PHASE_DONE
    preflight._manifest['completed_at'] = 1.0
...
def test_boot_reconcile_false_when_eligible_tar_is_pending_eligible_handoff(tmp_path, monkeypatch):
    daily_dir = tmp_path / 'daily'
    daily_dir.mkdir()
    tar_path = os.path.normpath(str(daily_dir / '2026-04-22.tar'))
    open(tar_path, 'wb').close()
    ...
    class _FakeCoord:
        ...
        def is_complete(self, _tar_path):
            return False
    monkeypatch.setattr('hpcperfstats.dbload.lib.sync_timedb_startup_day_close.build_unprocessed_raw_by_daily_tar', lambda *_a, **_k: {})
    monkeypatch.setattr('hpcperfstats.dbload.lib.sync_timedb_startup_day_close.days_ingest_complete_by_checkpoint', lambda *_a, **_k: [tar_path])
    monkeypatch.setattr('hpcperfstats.dbload.lib.sync_timedb_startup_day_close.days_quiescent_tar_needs_day_close_at_startup', lambda *_a, **_k: [])
    preflight = _make_preflight(tmp_path, _FakeCoord())
    preflight._manifest['phase'] = PHASE_DONE
    preflight._manifest['completed_at'] = 1.0
...
def test_boot_reconcile_true_when_new_eligible_not_in_pending_eligible(tmp_path, monkeypatch):
    daily_dir = tmp_path / 'daily'
    daily_dir.mkdir()
    tar_handoff = os.path.normpath(str(daily_dir / '2026-04-22.tar'))
    tar_new = os.path.normpath(str(daily_dir / '2026-04-23.tar'))
    open(tar_handoff, 'wb').close()
    ...
    class _FakeCoord:
        ...
        def is_complete(self, _tar_path):
            return False
    monkeypatch.setattr('hpcperfstats.dbload.lib.sync_timedb_startup_day_close.build_unprocessed_raw_by_daily_tar', lambda *_a, **_k: {})
    monkeypatch.setattr('hpcperfstats.dbload.lib.sync_timedb_startup_day_close.days_ingest_complete_by_checkpoint', lambda *_a, **_k: [tar_handoff, tar_new])
    monkeypatch.setattr('hpcperfstats.dbload.lib.sync_timedb_startup_day_close.days_quiescent_tar_needs_day_close_at_startup', lambda *_a, **_k: [])
    preflight = _make_preflight(tmp_path, _FakeCoord())
    preflight._manifest['phase'] = PHASE_DONE
    preflight._manifest['pending_eligible'] = [tar_handoff]
    preflight._manifest['pending_retry'] = []
    assert preflight._needs_boot_reconcile() is True

def test_discover_stall_watchdog_forces_done_with_pending_eligible_only(tmp_path, monkeypatch):
    ...
    class _FakeCoord:
        ...
        def recover_stale_manifest_entries(self):
            pass
    monkeypatch.setattr(cfg, 'get_sync_startup_discover_slice_stall_seconds', lambda: 60.0)
    preflight = _make_preflight(tmp_path, _FakeCoord())
    tar_path = os.path.normpath(str(tmp_path / 'daily' / '2026-05-25.tar'))
    with preflight._lock:
        preflight._manifest['phase'] = PHASE_DISCOVERING
        preflight._manifest['pending_eligible'] = [tar_path]
        preflight._manifest['pending_retry'] = []
        preflight._manifest['last_progress'] = 'discover_slice_begin'
        preflight._manifest['last_progress_at'] = time.time() - 120.0
    assert preflight._maybe_force_discover_done_on_stall() is True
    assert preflight.discover_done() is True
    assert preflight.pending_eligible_count() == 1

def test_discover_stall_watchdog_does_not_force_done_with_pending_retry(tmp_path, monkeypatch):
    ...
    with preflight._lock:
        ...
        preflight._manifest['last_progress'] = 'discover_slice_begin'
        preflight._manifest['last_progress_at'] = time.time() - 120.0
    assert preflight._maybe_force_discover_done_on_stall() is False
    assert preflight.discover_done() is False
