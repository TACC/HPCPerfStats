...
- **Graceful stop / `stopwaitsecs`:** `sync_timedb` cooperative drain is capped by **`SHUTDOWN_DRAIN_TIMEOUT_S` (120s)**. Unset supervisord **`stopwaitsecs`** defaults to **~10s**, then SIGKILL (and process-group kill for sync_timedb). The tracked config sets **`stopwaitsecs=130`** on **`hpcperfstats-rabbitmq-listener`**, **`sync_timedb`**, and **`update_metrics`**. Pair with compose **`stop_grace_period` ≥ 2m** (`HPCPERFSTATS_PIPELINE_STOP_GRACE`) and prefer `docker compose stop -t 180 pipeline` (rebuild script default stop timeout **300s**). Solo budgets: listend ~**20s**, update_metrics ~**30–60s**, sync_timedb up to **120s**. SIGTERM-driven exit **143** is the T0 expectation in [`OPERATOR_SYNC_TIMEDB_STALL_VERIFY.md`](OPERATOR_SYNC_TIMEDB_STALL_VERIFY.md). Avoid hard kill unless the container is wedged past grace.
