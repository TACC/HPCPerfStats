...
**Cold sealed day vs exit 124:** DB-complete catch-up on one calendar day can block many ingest workers on Redis L2 populate/wait while workers remain alive—this is **not** a dead pool, but imap sees zero completions until populate finishes. Mitigations (code): before each ingest chunk the supervisor **prewarms** Redis member maps once per unique calendar day (`Prewarming archive members Redis for day=…`); while **`in_flight_day_hint`** matches a day with active Redis populate progress, the stall counter **defers** (`WARN: pool imap stall deferred: Redis populate active for day=…`); ingest workers honor **`sync_ingest_per_file_timeout_s`** via a monotonic deadline in Redis wait loops (`ingest_ok=False` retry) even when SIGALRM is delayed in C extensions. Raising only `sync_pool_stall_abort_after_timeouts` without prewarm prolongs hangs without fixing the root cause.
