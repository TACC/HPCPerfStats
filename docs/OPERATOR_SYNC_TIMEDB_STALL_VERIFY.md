...
**Wave 3/4 parse-stage shares (optional):** when INI **`sync_ingest_parse_stage_telemetry=yes`**, `ingest file path=` lines add exhaustive stage tokens (1 decimal): **`lock_s`**, **`decode_s`**, **`feed_s`**, **`proc_merge_s`**, **`hw_df_s`**, **`proc_df_s`**, derived **`build_df_s`** (= merge+hw+proc frames), **`delta_s`**, **`collapse_s`**, **`arc_s`**, **`concat_s`**, **`start_s`**, **`stages_sum_s`**, and closed-book **`parse_unaccounted_s`** = `parse_elapsed_s - stages_sum_s - postgres_s` (combined ingest nests ORM inside the parse wall). Default is **off**. Rank product cuts from the largest hold share; if `parse_unaccounted_s` stays large after `postgres_s` subtraction, instrument further — do not start another coarse telem wave. Mid-cohort pastes must use **named-field** regex.
