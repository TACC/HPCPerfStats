...
- **`colima-docker-runtime.mdc`** — local Colima/Docker compose workflows are **disabled by default** (`HPCPERFSTATS_ENABLE_LOCAL_DOCKER=1` to re-enable); agents must not start Colima; dual-registered with compose workflow paths in `agent-discipline-core.mdc` + `hook_task_router.py`.
