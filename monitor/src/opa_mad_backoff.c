...
void opa_mad_note_failure(void)
{
  ...
  now = time(NULL);
  if (now > 0) {
    /* Warn once when first entering (or re-arming) the skip window. */
    if (g_opa_mad_skip_until <= 0 || now >= g_opa_mad_skip_until)
      arm_skip = 1;
    g_opa_mad_skip_until = now + OPA_MAD_BACKOFF_SEC;
  }
  if (arm_skip)
    monitor_log_warn("host_opa MAD: too many failures (%lu), skipping for %ds (sysfs continues)\n",
                     g_opa_mad_fail_streak, OPA_MAD_BACKOFF_SEC);
}
