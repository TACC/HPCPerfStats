...
| **pipeline** | `listend.py` | Pika consume on one BlockingConnection; **host-affine archive threads** (`listend_archive_worker_threads`, default **16**) for durable `current` append/rotate + threadsafe ack (cheap first-line/`$` host peek, raw AMQP bytes, ack before live-DB submit, sticky flock + kept `current` fd); drop-mode **`listend_amqp_prefetch`** (default **128**; pause keeps prefetch **1**). Optional live DB dual-write via host-affine **threads** (`listend_db_ingest_*`, default **32** threads, **8 GiB** total queue budget, **`listend_db_ingest_batch_samples=100`**, **`listend_db_ingest_flush_max_rows=2000`**, **`listend_db_ingest_flush_hold_s=5`**) sharing ordinary `queue.Queue` objects (no `/dev/shm`, no spawn). Flush is bounded by sample count, pending rows, or hold-age so a never-empty queue still writes Timescale. Backpressure when live on: **`listend_db_ingest_backpressure`** default **`drop`** (archive+ack, shed live DB via `queue_drops`) or **`pause`** (high watermark **0.95** / any worker full → stop consume without archive until resume **0.50**; overflow on broker). | Low archive path (parallelized across hosts); DB slots when live ingest on — size with `sync_ingest_pool_processes` against Postgres `max_connections`. When live on, archive-backed catch-up remains completeness repair for live gaps; archive/delete needs a sync_timedb file-complete mark |
...
| `sync_ingest_per_file_timeout_s` | 0 | Floor seconds for size-proportional per-file wall soft-kill (`0` disables wall B; idle stall owns soft-kill) |
| `sync_ingest_stall_idle_s` | 1800 | Soft-requeue when parse/write shows no progress for this many seconds (`0` disables) |
| `sync_ingest_per_file_timeout_s_per_mib` | (86400−900)/30720 | Added seconds per ceiling MiB when wall floor `> 0` (default maps **30 GiB → max**) |
...
**Backlog concurrency margin (2026-08):** default wall floor is **0** (SIGALRM soft-kill demoted). Soft-requeue uses **`sync_ingest_stall_idle_s`** (default **1800**) when parse/write heartbeats stop; Postgres **`statement_timeout`** remains the absolute DB ceiling. Coordinator submit-age ingest watchdog is **retired** (do not fall back to it). Apply on the **same redeploy** as the code release (no pre-code INI restart).

For large DB sites with slow duplicate-detection or bulk writes, prefer **idle stall** over wall B. Optionally re-enable size-proportional wall (`sync_ingest_per_file_timeout_s` floor + `per_mib`, capped by `max`) only when you need an absolute soft-kill; raise **`sync_pool_stall_abort_after_timeouts`** together with **`sync_ingest_per_file_timeout_max_s`** when increasing the per-file max. Dynamic pool imap abort follows the largest in-flight budget (or `max` when wall floor is 0). Catch-up mitigations for sealed-only archives: **`archive_keep_uncompressed_tar=yes`**, tune **`sync_ingest_pool_processes`**, and rely on store **single-flight populate** warming the day map (not N parallel `zstd` per worker). Ingest-time DLO quarantine for permanently corrupt raw is separate from exit **124**.
