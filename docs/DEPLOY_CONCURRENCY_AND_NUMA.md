...
Raw stats on disk remain the **source of truth** until validated archive membership, DB sampled-timestamp ingest gate (when enabled; stride = `sync_bulk_create_batch_size // 2` plus last line at EOF), and janitor raw removal all succeed.
...
| **Sample ingested, gaps between samples (M2)** | DB gate checks strided timestamp lines plus EOF last line, not every line; missing middle segments may still pass—tail-only partial ingest fails when last sample is absent. Treat unexpected raw deletion as incident-driven review, not routine ops. |
