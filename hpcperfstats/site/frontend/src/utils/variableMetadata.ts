...
export function normalizeVariableKey(name: unknown): string {
 ...
 */
export function getVariableTooltipContent(name: unknown) {
  const key = normalizeVariableKey(name);
  if (!key) return null;
  const entry = VARIABLE_METADATA[key as keyof typeof VARIABLE_METADATA] as
    | { description?: string; researcherUse?: string | null }
    | undefined;
  let description;
  ...
  const researcherUse =
    entry?.researcherUse && String(entry.researcherUse).trim()
...
export function getDescriptionForVariable(name: unknown) {
