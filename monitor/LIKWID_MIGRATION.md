...
- **Multi-group PERF collect:** after any group's `perfmon_setupCounters`, **every** LIKWID reader re-arms **its own** group before `readGroupCounters`:
  - **Core:** each `host_cpu_hw` tick → `likwid_pmc_adapter_prepare_collect()` → `setupCounters(g_group)` + `startCounters()` then `readGroupCounters(g_group)`.
  - **Uncore (IMC/DF):** each collect tick → `likwid_uncore_finish_group(group)` (same setup+start) then `readGroupCounters(group)`. Required on SKX/ICX where `uncore_imc` stops after core prepare; SPR/Turin may tolerate read-only collect when their PMU fds keep counting.
  - **RAPL PWR:** each collect tick → `setupCounters(g_pwr_group)` + `startCounters()` then `readGroupCounters(g_pwr_group)`; powercap fallback unchanged.
  Setup-only left Turin/SPR core flat after deploy; read-only uncore left SKX/ICX IMC CAS at zero. Judge AMD health on mid-row `retired_instructions` / `ls_dispatch` / `instr_retired` under load (not Intel FIXC/util).
