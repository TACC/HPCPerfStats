...
class TestJobListHistogramsView:
    ...
    def test_returns_401_when_not_authenticated(self):
        ...
        with patch("hpcperfstats.site.lib.machine.api.check_for_tokens", return_value=False):
    ...
    def test_rejects_legacy_queue_group(self):
        ...
        with patch("hpcperfstats.site.lib.machine.api.check_for_tokens", return_value=True):
            response = job_list_histograms(request)
...
def test_metric_group_null_plot_items_when_job_hist_returns_none():
    ...
    ), patch("hpcperfstats.site.lib.machine.api.check_for_tokens", return_value=True), patch(
        "hpcperfstats.site.lib.machine.api._build_histogram_queryset",
    ...
    ), patch("hpcperfstats.site.lib.machine.api.job_hist", return_value=None):
        response = job_list_histograms(request)
...
def test_build_histogram_queryset_matches_job_list_annotate_all():
    ...
    from hpcperfstats.site.lib.machine import api as api_module
...
def test_metric_group_calls_build_histogram_dataframe_when_jobs_present():
    ...
    with override_settings(
        ...
        "hpcperfstats.site.lib.machine.api.check_for_tokens", return_value=True
    ), patch(
...
def test_invalidate_job_browse_path_also_targets_job_list_api_cache():
    ...
    with patch("hpcperfstats.site.lib.machine.api._require_auth", return_value=None), patch(
        ...
        "hpcperfstats.site.lib.machine.api._delete_django_cache_page_entries_for_request",
    ) as mock_delete_django, patch(
        "hpcperfstats.site.lib.machine.api._full_page_cache_url_digests_for_request_paths",
        return_value=set(),
    ), patch(
...
class TestJobListNoHistogramsInResponse:
    ...
    def test_job_list_response_omits_script_and_div(self):
        ...
        from hpcperfstats.site.lib.machine.api import job_list
