...
- **`update_metrics` pool workers:** do **not** raise global `db_statement_timeout_ms` for long `host_data` compute. Use **`[PIPELINE] metrics_worker_statement_timeout_ms`** (default **0** = disable session timeout during `compute_metrics`; env **`HPCPERFSTATS_METRICS_WORKER_STATEMENT_TIMEOUT_MS`**). Per-job SIGALRM (`metrics_run_per_job_timeout_s` / stall timeout) remains the wall-clock backstop. Parent persist still uses **`metrics_persist_statement_timeout_ms`**. Metric row fetches split `host__in` chunks on statement timeout (same pattern as `jid_table`).
