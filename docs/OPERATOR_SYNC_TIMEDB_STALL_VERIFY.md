...
**Fail (T0 — stuck-inflight / ~1-core futex, hpcperfstats02 2026-09-13):** census `ingest=128/809x` (pool **full**) with `total_ingested` almost frozen; `parse_elapsed_s` hours ≫ `postgres_s`; host **~90% idle**; almost every ingest tid `wchan=futex_do_wait`. PATH `python3` GIL is **not** the daemon — require `readlink /proc/<pid>/exe` (`/opt/python3.14t/bin/python3.14t`). **Pass after lock/occupancy fix:** ingest ACKs move while pool is full; persist copy does not serialize waiters; fill cap ≤ `get_effective_cores()`; do not raise `sync_day_close_max_inflight`.
