...
**Wave 3/4 parse-stage shares (optional):** when INI **`sync_ingest_parse_stage_telemetry=yes`** **or** env **`HPCPERFSTATS_SYNC_INGEST_PARSE_STAGE_TELEMETRY=1`**, `ingest file path=` lines add exhaustive stage tokens (1 decimal): **`lock_s`**, **`decode_s`**, **`feed_s`**, **`proc_merge_s`**, **`hw_df_s`**, **`proc_df_s`**, derived **`build_df_s`** (= merge+hw+proc frames), **`delta_s`**, **`collapse_s`**, **`arc_s`**, **`concat_s`**, **`start_s`**, **`stages_sum_s`**, and closed-book **`parse_unaccounted_s`** = `parse_elapsed_s - stages_sum_s - postgres_s` (combined ingest nests ORM inside the parse wall). Default is **off** (do not bake ON). Rank product cuts from the largest hold share; if `parse_unaccounted_s` stays large after `postgres_s` subtraction, instrument further — do not start another coarse telem wave. Mid-cohort pastes must use **named-field** regex.

**Write-phase shares (optional, overnight decision):** when env **`HPCPERFSTATS_SYNC_INGEST_WRITE_TELEMETRY=1`**, the same `ingest file` lines also emit **`orm_materialize_s`**, **`orm_bulk_prep_s`**, **`db_execute_s`**, **`db_commit_s`**, and COPY detail **`copy_s`** / **`conflict_insert_s`** (nested inside execute; not double-counted in closed-book residual). Lines always include **`host_insert_arm=`** / **`proc_insert_arm=`**. Default is **off**. Pre-fix images can enable write telem and still **omit** write-phase tokens — treat missing `orm_materialize_s`/`db_execute_s` with telem on as undeployed log plumbing.

**Overnight decision soak (hpcperfstats04, post-COPY):** after CODE redeploy, manually set both env flags on pipeline (settings `environment:`), recreate, soak ≥8 h continuous. Morning: capture full pipeline log (no `--tail`), run `python3 scripts/measure_pipeline_ingest_rate.py --log-file … --since-minutes 480`, read **`decision_next=`** plus mid-tier postgres frac / top parse hold. Do **not** treat flat files/s as “write unchanged” (loaded48 showed postgres_s collapse with flat files/s). Do **not** raise ingest pool.
