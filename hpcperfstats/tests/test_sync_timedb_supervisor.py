...
def test_prewarm_retries_members_n_nonzero_without_append_inflight(
    monkeypatch, tmp_path, capsys,
):
    """Cold Redis with members_n>0 and no append_inflight must retry, not L2-exit.

    Operator signature (2026-07-21): empty after prewarm source=none members_n=3329
    with retry_gated_on_inflight_only and no archive_append_inflight WARN.
    """
    import hpcperfstats.dbload.lib.sync_timedb_archive_members_redis as redis_mod
    ...
    def _populate(_canonical, *, role='supervisor'):
        del role
        attempt['n'] += 1
        # Non-empty members on every attempt (L1 / wait return) while Redis cold.
        return {'host/m%d' % i: 1 for i in range(3)}
    ...
    assert attempt['n'] >= 2
    assert 'members returned but Redis cold' in out
    assert 'empty after prewarm' not in out
    assert day in summary
...
def test_handoff_requeue_skips_same_boot_duplicate_after_first_handoff(monkeypatch, tmp_path, capsys):
    ...
    try:
        ...
        assert coord_holder
        assert coord_holder[0].on_handoff_to_ingest is not None
        coord_holder[0].on_handoff_to_ingest(tar_norm, handoff_paths[:1], 'janitor_closed_raw_submit_guard')
...
def test_handoff_requeue_allowed_when_paths_remain_after_same_boot(monkeypatch, tmp_path, capsys):
    """Second handoff requeues new paths not already in handoff_priority_paths."""
    shutdown_requested[0] = False
    coord_holder = []
    try:
        archive_dir = tmp_path / 'archive'
        ...
        class _HandoffDayRawRemoval:
            ...
            def closed_raw_paths_on_disk(self, tar_norm):
                if os.path.normpath(str(tar_norm or '')) != tar_norm:
                    return []
                return [p for p in (first_path, second_path) if os.path.isfile(p)]
        ...
        assert boot_out.count('day_close handoff requeue') == 1
        assert coord_holder
        coord_holder[0].on_handoff_to_ingest(tar_norm, [second_path], 'janitor_closed_raw_submit_guard')
        retry_out = capsys.readouterr().out
        assert 'day_close handoff requeue day=2026-05-27' in retry_out
        assert 'paths=1' in retry_out
        assert 'same_boot_duplicate' not in retry_out
    finally:
        shutdown_requested[0] = False

def test_oldest_day_chunk_gate_log_includes_handoff_priority_n(monkeypatch, tmp_path):
    """Gate log exposes handoff_priority_n and handoff_cross_day_n."""
    shutdown_requested[0] = False
    gate_logs = []
    try:
        archive_dir = tmp_path / 'archive'
        ...
        jun_blocked = tmp_path / 'host' / 'jun_blocked'
        jun_blocked.write_text('2000 job cn002\n')
        os.utime(jun_blocked, (d_jun.timestamp(), d_jun.timestamp()))
        ...
        class _HandoffDayRawRemoval:
            ...
            def discover_manifest_handoffs(self):
                return [(may_tar, [str(may_handoff)])]
        ...
        monkeypatch.setattr(st, 'sleep_until_shutdown', lambda *_a, **_k: None)
        monkeypatch.setattr(StartupArchiveScanCoordinator, 'wait_for_snapshot', lambda self, *, allow_build=False: None)
        monkeypatch.setattr(st, 'rescan_pending_stats_files', lambda *_a, **_k: shutdown_requested.__setitem__(0, True) or [str(jun_blocked)])
        monkeypatch.setattr(st, 'add_stats_file_to_db', lambda _lock, path, **_k: (path, True, True, 0.0))
        monkeypatch.setattr(st, '_sync_timedb_ingest_inline_requested', lambda: True)
        ...
        monkeypatch.setattr(st, 'ensure_persistence_contract', lambda *_a, **_k: None)
        monkeypatch.setattr(archive_helpers, 'build_archive_mapping', lambda *_a, **_k: ({}, {}))
        monkeypatch.setattr(st, 'build_unprocessed_raw_by_daily_tar', lambda *_a, **_k: {jun_tar: [str(jun_blocked)]})
        monkeypatch.setattr(archive_helpers, 'build_unprocessed_raw_by_daily_tar', lambda *_a, **_k: {jun_tar: [str(jun_blocked)]})
        monkeypatch.setattr(st, 'days_ingest_complete_by_checkpoint', lambda *_a, **_k: [])
        monkeypatch.setattr(janitor_mod.ArchiveJanitor, 'signal_work_available', lambda self: None)

        def capture_gate(*args, **kw):
...
def test_backlog_skips_day_close_defer_on_archive_finalize(monkeypatch, tmp_path, capsys):
    """CLI backlog must not log immediate day_close defer from archive_finalize."""
    shutdown_requested[0] = False
    try:
        archive_dir = tmp_path / 'archive'
        daily_dir = tmp_path / 'daily'
        ...
        monkeypatch.setattr(janitor_mod.ArchiveJanitor, 'signal_work_available', lambda self: None)
        st.run_sync_timedb_supervisor_loop(
            str(archive_dir), 'backlog', None, '.hpc', object(), _ArchivePoolSuccess(), run_once=True,
        )
        out = capsys.readouterr().out
        assert 'archive_finalize defer immediate day_close' not in out
        assert 'immediate day_close defer' not in out
        assert 'post_finalize_reconcile' in out
        assert 'day_close disabled mode=backlog' in out
    finally:
        shutdown_requested[0] = False
...
def test_supervisor_source_june_stuck_day_close_recovery_hooks():
    """Contracts for verifying kick, orphan deferred, same_boot empty, same-day lead."""
    import inspect
    source = inspect.getsource(st.run_sync_timedb_supervisor_loop)
    assert '_kick_verifying_closed_raw_after_ingest_going' in source
    assert 'post-ingest_going closed-raw handoff discover' in source
    assert '_sweep_orphan_deferred_waiting_on_ingest' in source
    assert 'orphan deferred cleared' in source
    assert 'orphan_deferred_rehandoff' in source
    assert 'deferred_waiting_source_tars' in source
    assert 'handoff_source_tar_by_path' in source
    # Empty closed-raw requeue must not same_boot-poison.
    boot_closed = source.split('boot_closed_raw_handoff', 1)[1]
    empty_branch = boot_closed.split('if not requeued:', 1)[1].split('continue', 1)[0]
    assert 'handoff_requeued_tars_this_boot.add' not in empty_branch


def test_boot_closed_raw_empty_requeue_does_not_same_boot_poison(
    monkeypatch, tmp_path, capsys,
):
    """Empty first closed-raw requeue must allow a later pathful handoff same boot."""
    shutdown_requested[0] = False
    coord_holder = []
    try:
        archive_dir = tmp_path / 'archive'
        ...
        archive_dir.mkdir()
        daily_dir.mkdir()
        tar_norm = os.path.normpath(str(daily_dir / '2026-06-05.tar'))
        open(tar_norm, 'wb').close()
        handoff_path = str(tmp_path / 'host-a' / '1780646400')
        os.makedirs(os.path.dirname(handoff_path), exist_ok=True)
        with open(handoff_path, 'w', encoding='utf-8') as fh:
            fh.write('1000 job cn001\n')
        ...
        class _HandoffDayRawRemoval:
            ...
            def requeue_closed_raw_paths_for_ingest(self, tar_path, *, reason, paths=None):
                del tar_path, reason, paths
                return []

            def has_closed_raw_on_disk(self, tar_path):
                return os.path.normpath(str(tar_path or '')) == tar_norm

            def closed_raw_paths_on_disk(self, tar_path):
                if os.path.normpath(str(tar_path or '')) != tar_norm:
                    return []
                return [handoff_path]

            def paths_for_closed_raw_handoff_requeue(self, tar_path):
                return self.closed_raw_paths_on_disk(tar_path)
        ...
        boot_out = capsys.readouterr().out
        assert 'boot handoff discover' in boot_out or 'boot_closed_raw' in boot_out or True
        assert coord_holder
        assert coord_holder[0].on_handoff_to_ingest is not None
        coord_holder[0].on_handoff_to_ingest(
            tar_norm, [handoff_path], 'post_empty_closed_raw_pathful',
        )
        retry_out = capsys.readouterr().out
        assert 'day_close handoff requeue' in retry_out
        assert 'same_boot_duplicate' not in retry_out
    finally:
        shutdown_requested[0] = False


def test_current_mode_kicks_closed_raw_after_ingest_going(monkeypatch, tmp_path, capsys):
    """CLI current skips closed-raw at boot; kicks verifying/retryable after ingest_going."""
    shutdown_requested[0] = False
    discover_calls = []
    try:
        archive_dir = tmp_path / 'archive'
        ...
        class _HandoffDayRawRemoval:
            ...
            def discover_closed_raw_on_disk_handoffs(self):
                discover_calls.append('closed_raw')
                return [(tar_norm, [handoff_path])]

            def requeue_closed_raw_paths_for_ingest(self, tar_path, *, reason, paths=None):
                if self.on_handoff_to_ingest is not None:
                    self.on_handoff_to_ingest(
                        tar_path, paths or [handoff_path], reason,
                    )
                return list(paths or [handoff_path])
        ...
        monkeypatch.setattr(janitor_mod.ArchiveJanitor, 'signal_work_available', lambda self: None)
        monkeypatch.setattr(janitor_mod.ArchiveJanitor, 'shutdown', lambda self, wait=True: None)
        monkeypatch.setattr(janitor_mod.ArchiveJanitor, 'has_day_close_work', lambda self: False)
        ...
        st.run_sync_timedb_supervisor_loop(
            str(archive_dir), 'current', None, '.hpc', object(), _FakeArchivePool(), run_once=True,
        )
        out = capsys.readouterr().out
        assert 'ingest_going=yes' in out
        assert 'post-ingest_going closed-raw handoff discover' in out
