 ...
 *  Intel BDW CBo uncore via LIKWID PMON.
 */
...
{
  return likwid_uncore_adapter_begin(type, LIKWID_UNCORE_PROFILE_CBO_BDW);
}

static void intel_bdw_cbo_collect(struct stats_type *type)
{
  likwid_uncore_adapter_collect(type, LIKWID_UNCORE_PROFILE_CBO_BDW);
}

struct stats_type intel_bdw_cbo_stats_type = {
  .st_name = "intel_x86_uncore_cbo_bdw",
