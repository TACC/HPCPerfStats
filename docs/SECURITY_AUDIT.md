...
| 3 | CSP allows untrusted script execution | **Yes (defense-in-depth)** | Policies allowed `unsafe-inline` (and Bokeh `unsafe-eval`). | Build-time SHA-256 hashes for **script-src**; no `script-src 'unsafe-inline'`; machine keeps justified `unsafe-eval` only; JSON/redirects use no-active CSP. | Bokeh `unsafe-eval` until CustomJS-free embed. |
