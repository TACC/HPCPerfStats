...
| **pipeline** | `listend.py` | Pika + archive write; optional live DB dual-write via host-affine spawn pool (`listend_db_ingest_*`, default **30** workers, **8 GiB** total queue budget, **`listend_db_ingest_batch_samples=100`**). When live on: **`basic_qos(prefetch_count=1)`**; if DB queues hit the high watermark (**0.95** of budget, or any worker full), **stop RabbitMQ consume** (no archive) until usage falls to the resume watermark (**0.50**); overflow sits on the broker ready queue (not dropped). | Low archive path; DB slots when live ingest on — size with `sync_ingest_pool_processes` against Postgres `max_connections`. When live on, sync_timedb disables weak head/tail (and tail-window) `db_skip` probes; completeness uses duplicate-timestamp scan; archive/delete needs a sync_timedb file-complete mark |
