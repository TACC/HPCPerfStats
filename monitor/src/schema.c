/* Build and tear down schema tables from whitespace-separated entry definitions. */
#include <malloc.h>
#include <stdlib.h>
#include <string.h>

#include "dict.h"
#include "schema.h"
#include "string1.h"
#include "trace.h"

/* Free partially built schema after dict_set/parse/calloc failure. */
static void schema_init_abort(struct schema *sc)
{
  if (sc == NULL)
    return;

  if (sc->sc_dict.d_table != NULL) {
    size_t j = 0;
    char *key;

    while ((key = dict_for_each(&sc->sc_dict, &j)) != NULL) {
      struct schema_entry *se = key_to_schema_entry(key);

      free(se->se_unit);
      free(se->se_desc);
      free(se);
    }
    dict_destroy(&sc->sc_dict, NULL);
  }
  free(sc->sc_ent);
  sc->sc_ent = NULL;
  sc->sc_len = 0;
  memset(&sc->sc_dict, 0, sizeof(sc->sc_dict));
}

static int schema_fill_entry_array(struct schema *sc)
{
  size_t i = 0;
  char *key;

  sc->sc_ent = (struct schema_entry **)calloc(sc->sc_len, sizeof(*sc->sc_ent));
  if (sc->sc_ent == NULL && sc->sc_len != 0) {
    ERROR("cannot allocate schema entries: %m\n");
    return -1;
  }

  while ((key = dict_for_each(&sc->sc_dict, &i)) != NULL) {
    struct schema_entry *se = key_to_schema_entry(key);

    sc->sc_ent[se->se_index] = se;
    TRACE("i %zu, d_key `%s', se_key `%s', se_index %u\n", i, key, se->se_key, se->se_index);
  }
  return 0;
}

int schema_init(struct schema *sc, const char *def)
{
  int rc = -1;
  size_t nr_se = 0;
  char *cpy = NULL;
  char *str = NULL;
  char *tok = NULL;

  if (sc == NULL || def == NULL)
    return -1;

  cpy = strdup(def);
  if (cpy == NULL) {
    ERROR("cannot duplicate schema definition: %m\n");
    return -1;
  }
  str = cpy;

  if (dict_init(&sc->sc_dict, 0) < 0) {
    ERROR("cannot initialize schema: %m\n");
    goto err;
  }

  while ((tok = wsep(&str)) != NULL) {
    struct schema_entry *se = parse_schema_entry(tok);

    if (se == NULL)
      goto err_abort;

    se->se_index = (unsigned int)nr_se++;
    if (dict_set(&sc->sc_dict, se->se_key) < 0) {
      free(se->se_unit);
      free(se->se_desc);
      free(se);
      goto err_abort;
    }
  }

  sc->sc_len = nr_se;
  if (schema_fill_entry_array(sc) < 0)
    goto err_abort;

  rc = 0;
  goto err;

err_abort:
  schema_init_abort(sc);
err:
  free(cpy);
  return rc;
}

void schema_destroy(struct schema *sc)
{
  size_t i;

  if (sc == NULL)
    return;

  for (i = 0; i < sc->sc_len; i++) {
    struct schema_entry *se = sc->sc_ent[i];

    free(se->se_unit);
    free(se->se_desc);
    free(se);
  }
  free(sc->sc_ent);
  dict_destroy(&sc->sc_dict, NULL);
  memset(sc, 0, sizeof(*sc));
}
