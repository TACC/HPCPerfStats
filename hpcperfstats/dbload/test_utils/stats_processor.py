...
exclude_typs = [
    "block", "ib", "ib_sw", "intel_skx_cha", "mdc", "numa", "osc", "proc", "ps",
    "sysv_shm", "tmpfs", "vfs", "vm"
]
...
def process(stats_file):
  ...
  stats["dif"] = (stats.groupby(["host", "jid", "typ", "dev",
                                 "eve"])["val"].diff()).fillna(0)

  # correct stats for rollover and units
  stats["dif"].mask(stats["dif"] < 0,
                    2**stats["wid"] + stats["dif"],
                    inplace=True)
  stats["dif"] = stats["dif"] * stats["mult"]
  del stats["wid"], stats["mult"]

  # aggregate over devices
  stats = stats.groupby(["time", "host", "jid", "typ", "eve",
...
def agg(df, typename, event_list, tags):
  """Aggregate df by tags for the given typename and event_list (sum).
    ...
    AI generated.
    """
  datam = df[(df["typ"].values == typename) &
             (df["eve"].isin(event_list))].groupby(tags).sum()
  return datam
...
for entry in os.scandir(directory):
  if entry.is_file() or not entry.name.startswith("c"):
    continue
...
# Lustre MDS Usage
event_list = [
    "open", "close", "mmap", "seek", "fsync", "setattr", "truncate", "flock",
    "getattr", "statfs", "alloc_inode", "setxattr", "getxattr", "listxattr",
    "removexattr", "inode_permission", "readdir", "create", "lookup", "link",
