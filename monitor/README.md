...
- **`cpu_counter_metrics` (DCGM + PAPI hybrid on Grace)**: DCGM publishes util accumulators and **`dcgm_cpu_power_util_w`** / **`dcgm_cpu_power_limit_w`** (fields 1130/1131 on **`DCGM_FE_CPU`**; per-socket watts repeated on each logical CPU in that socket). Statically linked **PAPI** (pin **7.2.0**) fills **`aperf`/`mperf`/`cpu_clock_est_cycles`**, **`fp_arith_inst_retired_scalar_single`**, **`fp_arith_inst_retired_scalar_double`**, **`arm_est_flops`** (= SP+DP only), and Neoverse **`arm_int8_ops`** / **`arm_int16_ops`** (`ASE_SVE_INT8_SPEC` / `ASE_SVE_INT16_SPEC`). DCGM synthetic FLOPs/cycles are disabled when PAPI is compiled in.
