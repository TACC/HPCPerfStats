...
4. **Compose file:**
   ...
   **Daily monitor archive compression:** `sync_timedb` seals each day’s `YYYY-MM-DD.tar` to **`YYYY-MM-DD.tar.zst`** with **zstd** (always `--long=31` on compress, decompress, test, and stream; see `[PORTAL]` keys `archive_zstd_level`, `archive_zstd_threads` in `hpcperfstats.ini.example`). Large daily tars can be sealed with zstd’s long-distance mode; decoders must pass the same flag or decompression fails even when the compressed file is small. When inspecting archives by hand, use for example `zstd -d --long=31 -o YYYY-MM-DD.tar YYYY-MM-DD.tar.zst` (legacy `.tar.gz` uses `zstd -d --format=gzip` instead—no `--long`). Before raising `archive_zstd_level` above **9** on production data, benchmark a representative daily tar on the pipeline host: `zstd -b3 -e12 -T0 -S -- ./YYYY-MM-DD.tar`.
