...
- **`census`** — 60s depth; ratios are **inflight/queued**; `busy=` replaces opaque `local=I/A/D`. **Always** includes `total_ingested=` (process-lifetime successful `_drain_ingest_ready` `ingested` ACKs, including `0`; not db_skip). A **rising** value across consecutive 60s lines means files completed even when `ingest=0/0`. A **frozen** value with `ingest=0/0` is idle ingest (not a dead reconstruct-coordinator). Supervisor restart resets the counter to 0.
- **Stuck-inflight (hpcperfstats02 2026-09-13):** `ingest=pool/deep` (pool **full**, e.g. `128/809x`) plus **frozen** `total_ingested` is a **completion-rate** stall, not RC7–RC9 underfill. Pair with `parse_elapsed_s` hours, host CPUs **not** at `nproc`, thread `wchan=futex_do_wait`, and **`readlink /proc/<sync_timedb pid>/exe`**. PATH `python3` is GIL and is **not** the daemon. Sticky `discover=1/0` does **not** occupy ingest slots. Do **not** raise `sync_day_close_max_inflight`.
