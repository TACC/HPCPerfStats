...
describe("HostDetail", () => {
  ...
  it("shows loading while the host plot request is in flight", () => {
    vi.spyOn(apiModule.api, "getHostPlot").mockReturnValue(new Promise(() => {}));
    renderHostDetail();
  ...
  it("embeds the host plot when plot_item is returned", async () => {
    const embedItem = vi.fn(() => ({ roots: [{ model: { document: { is_idle: true, idle: { connect: vi.fn(), disconnect: vi.fn() } } } }] }));
    window.Bokeh = { embed: { embed_item: embedItem } };

    vi.spyOn(apiModule.api, "getHostPlot").mockResolvedValue({
      host: "node1.cluster",
      plot_item: { doc: {}, root_ids: ["host-root"] },
  ...
  it("shows a banner when the API request fails", async () => {
    vi.spyOn(apiModule.api, "getHostPlot").mockRejectedValue(new Error("Host plot API failed"));
    renderHostDetail();
    await waitFor(() => {
