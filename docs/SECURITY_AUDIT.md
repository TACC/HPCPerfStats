...
Internal reference for security posture review. **Last reviewed:** 2026-09-03 (frontend npm pin refresh + prior Dependabot #110–#116 / Bokeh 3.10 lockstep; Burp Suite report disposition + nginx HSTS/OCSP/CSP edge hardening 2026-08-05; frontend stack posture 2026-07-31; Dependabot npm sweep 2026-07-26).
...
2. **pip-audit** inside the production **web** container image (`podman run --rm hpcperfstats pip-audit`, 2026-06-05): **no known vulnerabilities** in installed runtime deps (Django 6.0.6, cryptography 48.0.0, requests 2.34.2, pillow 12.2.0). Host `.venv` freeze (dev/test extras) still reports low-severity **idna** / **pip** advisories not present in the production image.
3. **npm audit** in `hpcperfstats/site/frontend` (2026-09-03 pin refresh): **0** reported vulnerabilities (739 packages) after raising direct floors (`next@^16.3.4`, `vitest@^5`, Orval **`8.27.0`** exact, etc.) and override floors (`dompurify@^3.4.14`, `esbuild@^0.28.2`, `js-yaml@4.3.2`, …). Prior 2026-09-02: **0** after Dependabot #110–#116 — `fast-uri@^4.1.3` (lock **4.1.4**), `qs@^6.16.0`, `@xmldom/xmldom@^0.9.12`. Prior 2026-08-13: **0** after Dependabot #109 — `nanoid@^3.3.18` override (GHSA-2v37-7h3g-55p8 / CVE-2026-67213; lock was 3.3.17). Prior 2026-08-03: **0** after pin refresh — raised floors plus new `overrides` for `hono`, `ip-address`, and `undici`. Prior 2026-07-26: **0** after Dependabot sweep — `next@^16.2.11` plus raised/added `overrides` for `dompurify`, `postcss`, `brace-expansion`, `fast-uri`, `sharp`, and `@hono/node-server`. Prior 2026-06-15: **0** after `dompurify` / `js-yaml` overrides.
...
**2026-08-05:** Docker `frontend-builder` upgrades to **npm 12.0.2** before `npm ci` (stock Node 26 image still ships npm 11). npm 12 blocks dependency lifecycle scripts unless listed in `package.json` `allowScripts` (currently pinned `esbuild@0.28.2` only; was `0.28.1`). Aligns with Shai-Hulud / install-time worm defenses. Same day: reverted accidental `js-yaml@^5` override to **`4.x`** (Orval default-import breakage in `build:prod`; floor now **`4.3.2`**).
...
- `robots.txt` defaults to **`Disallow: /`** with explicit **`Allow:`** prefixes only for anonymous **`/pub/`** HTML shell paths (canonical registry in [`publicRobotsAllowPrefixes.ts`](../hpcperfstats/site/frontend/src/config/publicRobotsAllowPrefixes.ts), emitted by **`scripts/generate-robots-txt.mjs`** during the Next static-export build; nginx serves **`/robots.txt`** from **`STATIC_ROOT`**); **`/api/pub/`** remains uncrawlable by default.
...
| 2026-05-07 | **`robots.txt`**: nginx serves a build-emitted static file; Allow-list registry is **`publicRobotsAllowPrefixes.ts`** (edge headers in **`nginx-edge-security-headers.inc`**). |
...
| 2026-09-03 | Frontend npm pin refresh: Next 16.3.4, Vitest 5, Orval exact 8.27.0 (8.28 incomplete on npm), override floors (`esbuild` 0.28.2, `js-yaml` 4.3.2, `dompurify` 3.4.14, …); `npm audit` 0 (739 pkgs); Vitest 712 green. |
