...
**sync_timedb ingest archive member cache (host, no compose):** on slow cloud-sync checkouts prefer `scripts/run_tests.py --no-django` (direct `pytest` on a virtiofs/cloud-sync tree can hang during collection). Targeted regressions:
