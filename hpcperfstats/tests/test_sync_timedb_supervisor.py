...
def test_archive_finalize_skips_invalidate_when_tar_append_redis_merge_succeeded(
    monkeypatch,
):
  shutdown_requested[0] = False
  invalidated = []
...
def test_supervisor_opens_ingest_when_discover_done_pending_eligible_only(
    monkeypatch,
):
  """Async-deferred pending_eligible must not keep the startup ingest gate closed."""
  shutdown_requested[0] = False
  rescan_calls = {"n": 0}
  ingest_calls = {"n": 0}
  ...
  class _PhaseDonePendingEligible:
    enabled = True
    ...
    def phase(self):
      return day_close_mod.PHASE_DONE
    ...
    def pending_eligible_count(self):
      return 2
  ...
  try:
    ...
    monkeypatch.setattr(preflight_mod, "StartupRawRemovalPreflight", _DoneRawPreflight)
    monkeypatch.setattr(st, "StartupRawRemovalPreflight", _DoneRawPreflight)
    monkeypatch.setattr(day_close_mod, "StartupDayClosePreflight", _PhaseDonePendingEligible)
    monkeypatch.setattr(st, "StartupDayClosePreflight", _PhaseDonePendingEligible)
    monkeypatch.setattr(tail_mod, "StartupTailIngestCoordinator", _DoneTailIngest)
    monkeypatch.setattr(st, "StartupTailIngestCoordinator", _DoneTailIngest)
    monkeypatch.setattr(
        StartupArchiveScanCoordinator,
...
def test_supervisor_reconcile_cap_uses_coordinator_snapshot_no_live_collect(
    monkeypatch, capsys,
):
  """Startup drain cap must not force_full_scan when coordinator snapshot exists."""
  shutdown_requested[0] = False
  collect_calls = {"n": 0}
  ...
  class _EnabledDoneRawPreflight:
    enabled = True
  ...
  try:
    ...
    monkeypatch.setattr(preflight_mod, "StartupRawRemovalPreflight", _EnabledDoneRawPreflight)
    monkeypatch.setattr(st, "StartupRawRemovalPreflight", _EnabledDoneRawPreflight)
    monkeypatch.setattr(day_close_mod, "StartupDayClosePreflight", _PhaseDoneDiscover)
    monkeypatch.setattr(st, "StartupDayClosePreflight", _PhaseDoneDiscover)
    monkeypatch.setattr(tail_mod, "StartupTailIngestCoordinator", _DoneTailIngest)
    monkeypatch.setattr(st, "StartupTailIngestCoordinator", _DoneTailIngest)
    monkeypatch.setattr(
        StartupArchiveScanCoordinator,
        "get_snapshot",
        lambda self: coordinator_snapshot,
    )
    monkeypatch.setattr(
        StartupArchiveScanCoordinator,
        "is_startup_heavy_maintenance_idle",
    ...
    assert "pending reconcile cap begin source=coordinator" in out
    assert "startup maintenance idle; ingest may begin" in out
  finally:
    shutdown_requested[0] = False


def test_supervisor_blocks_ingest_when_pending_retry(monkeypatch):
  """Discover retry deferrals must keep the startup ingest gate closed."""
  shutdown_requested[0] = False
  rescan_calls = {"n": 0}
  ingest_calls = {"n": 0}
  ...
  class _PendingRetryDiscover:
    ...
    def phase(self):
      return day_close_mod.PHASE_DISCOVERING
  ...
  try:
    ...
    monkeypatch.setattr(preflight_mod, "StartupRawRemovalPreflight", _DoneRawPreflight)
    monkeypatch.setattr(st, "StartupRawRemovalPreflight", _DoneRawPreflight)
    monkeypatch.setattr(day_close_mod, "StartupDayClosePreflight", _PendingRetryDiscover)
    monkeypatch.setattr(st, "StartupDayClosePreflight", _PendingRetryDiscover)
    monkeypatch.setattr(tail_mod, "StartupTailIngestCoordinator", _DoneTailIngest)
    monkeypatch.setattr(st, "StartupTailIngestCoordinator", _DoneTailIngest)
    monkeypatch.setattr(
        st.cfg,
...
def test_immediate_day_close_submits_checkpoint_complete_not_chunk_touched_only(
    monkeypatch,
    tmp_path,
):
  shutdown_requested[0] = False
...
def test_prewarm_skips_sealed_scan_after_tar_append_merge(
    monkeypatch, tmp_path,
):
  ...
  tar.write_bytes(b"t")
  canonical = str(zst)
  cache_key = archive_helpers._daily_archive_members_cache_key(
  ...
  monkeypatch.setattr(
      st, "get_existing_archive_members_for_daily_archive", _track_get_existing,
  )
  monkeypatch.setattr(
      st,
      "log_print",
      lambda *args, **kw: log_lines.append(" ".join(str(a) for a in args)),
  )
...
def test_supervisor_oldest_day_chunk_gate_inflight_starvation(
    monkeypatch, tmp_path,
):
  ...
  try:
    ...
    monkeypatch.setattr(
        st,
        "build_unprocessed_raw_by_daily_tar",
        lambda *_a, **_k: {tar_a: list(handoff)},
    )
    monkeypatch.setattr(
        archive_helpers,
        "build_unprocessed_raw_by_daily_tar",
...
def test_supervisor_chunk_gate_unblocks_when_blocked_excluded_from_processed(
  ...
  try:
    ...
    monkeypatch.setattr(
        st,
        "build_unprocessed_raw_by_daily_tar",
        lambda *_a, **_k: {tar_a: list(blocked)},
    )
    monkeypatch.setattr(
        st,
        "build_live_unprocessed_by_tar_for_reconcile",
...
def test_handoff_giant_day_slice_ingest(monkeypatch, tmp_path):
  ...
  try:
    ...
    monkeypatch.setattr(
        st.cfg,
        "get_sync_startup_tail_ingest_max_files",
        lambda: 100,
    )
    ...
    begin_logs = [
        line for line in log_lines if "startup handoff giant-day ingest begin" in line
    ]
    assert begin_logs
...
def test_closed_raw_handoff_uses_steady_chunk_not_giant_day_slice(
    monkeypatch, tmp_path, capsys,
):
  ...
  try:
    ...
    monkeypatch.setattr(st, "_sync_timedb_ingest_inline_requested", lambda: True)
    monkeypatch.setattr(
        st, "add_stats_file_to_db", lambda _lock, path, **_k: (
            ingest_calls.append(path) or (path, True, True, 0.0)
        ),
    )
    monkeypatch.setattr(
        st.cfg,
...
def test_giant_day_slice_gated_to_startup_handoff_recover_only(
    monkeypatch, tmp_path, capsys,
):
    ...
    class _ArchivePoolSuccess:
      def map_async(self, _fn, items):
        class _R:
          ...
          def get(self):
            return [
                st.ArchiveAppendOutcome(redis_merge_ok=False)
                for _ in items
            ]
...
def test_post_finalize_reconcile_clears_blocked_before_handoff(
    monkeypatch, tmp_path, capsys,
):
...
def test_handoff_giant_day_does_not_block_main_on_immediate_day_close(
    monkeypatch, tmp_path, capsys,
):
...
def test_startup_handoff_no_giant_slice_manifest_done_0522(
    monkeypatch, tmp_path, capsys,
):
...
def test_startup_same_boot_duplicate_kicks_delete_not_skip(
    monkeypatch, tmp_path, capsys,
):
...
def test_gate_blocked_n_excludes_db_complete_paths(
    monkeypatch, tmp_path, capsys,
):
  ...
  try:
    ...
    accrual_snapshot = ArchiveMaintenanceSnapshot(
        closed_paths=blocked,
        mapping={str(daily_dir / "2020-01-01.tar.zst"): list(blocked)},
        first_timestamp_by_path={blocked[0]: 1.0, blocked[1]: 1.0},
    )
    ...
    def fake_add(_lock, path, **_kwargs):
      if not checkpoint_seeded["done"] and path in blocked:
        ...
        if fp is not None:
          st.run_sync_timedb_supervisor_loop  # keep st referenced
      return (path, False, True, 0.0)
    ...
    monkeypatch.setattr(
        ...
        lambda _checkpoint_path, checkpoint_entries=None: set(blocked),
    )
    ...
    combined = out + "\n".join(gate_logs)
    assert (
        "oldest_day_chunk_gate_all_db_complete" in combined
        or any("blocked_n=0" in line for line in gate_logs)
        or "blocked_n=0" in combined
    )
  finally:
    shutdown_requested[0] = False


def test_chunk_reconcile_single_live_scan_per_chunk(
    monkeypatch, tmp_path, capsys,
):
  ...
  try:
    ...
    # Pre-chunk rescan reconcile + one chunk reconcile; chunk gate reuses cache (2 not 3).
    assert live_reconcile_calls == [1, 1]
  finally:
    shutdown_requested[0] = False


def test_handoff_light_when_drain_disabled(
    monkeypatch, tmp_path, capsys,
):
...
def test_chunk_end_defers_immediate_day_close_when_handoff_pending(
    monkeypatch, tmp_path, capsys,
):
...
def test_chunk_end_defers_immediate_day_close_when_closed_raw_guard(
    monkeypatch, tmp_path, capsys,
):
  ...
  try:
    ...
    monkeypatch.setattr(
        ...
        lambda *_a, **_k: (False, "closed_raw_on_disk"),
    )
    monkeypatch.setattr(
        janitor_mod.ArchiveJanitor,
        "signal_work_available",
    ...
    out = capsys.readouterr().out
    assert (
        "immediate day_close defer context=chunk_end reason=closed_raw_guard"
        in out
    )
    assert "chunk ingest summary" in out
  finally:
...
def test_arch_june04_handoff_after_giant_finalize_dispatches_chunk(
  ...
  try:
    ...
    assert (
        "archive_finalize defer immediate day_close reason=handoff_priority"
        in out
        or "immediate day_close defer context=chunk_end reason=handoff_priority"
        in out
    )
