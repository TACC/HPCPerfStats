...
| **`sync_process_tree_rss_limit_mb`** / **`sync_process_tree_rss_exit_mb`** (default defer **96000** MiB ≈ 75% of **128 GiB** cgroup) | Defer chunks or exit **137** on **supervisor + pool worker** RSS |
...
**Catch-up INI starting points** (until backlog of multi‑GiB segments clears): defaults above ship in **`hpcperfstats.ini.example`** with the release — **`sync_pool_process_cap=8`**, **`sync_ingest_pool_maxtasksperchild=1`**, **`sync_ingest_malloc_trim_after_file=yes`**, **`sync_process_tree_rss_limit_mb=96000`**. On **small dev compose** hosts (e.g. 8–16 GiB pipeline cgroup), set **`sync_process_tree_rss_limit_mb=0`** to disable tree defer and optionally raise **`sync_pool_process_cap`** if ingest is CPU-bound. Historical catch-up tuning: `sync_ingest_chunk_size=8–16`. On very large trees (~100k+ closed paths), optional interim disable until fixes ship: `sync_startup_day_close_preflight=no` (janitor **`DAY_CLOSE`** debt remains). After startup-scan + janitor budget fixes, prefer leaving prefights **on** and tune `sync_startup_snapshot_wait_seconds` (default **300**), `archive_janitor_days_per_tick`, `sync_day_close_async_workers`. Set **`sync_startup_drain_day_close_before_ingest=no`** only when startup latency from day-close drain is unacceptable (ingest may race async DAY_CLOSE).
...
Raw stats on disk remain the **source of truth** until validated archive membership, DB ingest gate (when enabled; default **`sync_archive_db_ingest_gate_mode=head`** checks first timestamp only; **`sample`** adds strided lines plus EOF last line), and janitor raw removal all succeed.
...
| **Stuck janitor debt after crash** | Inspect **`.sync_archive_maint_hints.json`** v2 (`debt_queue`, `day_phases`); restart **`sync_timedb`**. Scheduled maintenance refreshes snapshot/hints only — it does **not** enqueue new **`DAY_CLOSE`** debt; async day-close submit comes from startup discover and checkpoint-complete supervisor hooks. |
...
| **Sample ingested, gaps between samples (M2)** | Applies when **`sync_archive_db_ingest_gate_mode=sample`**: gate checks strided timestamp lines plus EOF last line, not every line; missing middle segments may still pass—tail-only partial ingest fails when last sample is absent. Treat unexpected raw deletion as incident-driven review, not routine ops. |
...
| **Quiescent dirty `.tar` at startup** | When a daily **`.tar`** is dirty but **no closed raw remains on disk** for that calendar day (including stale checkpoint-unprocessed paths with files already gone), **`StartupDayClosePreflight`** submits async **`DAY_CLOSE`** with **`reason=startup_quiescent_tar`** — same coordinator as checkpoint-complete startup close. Requires filesystem truth (`os.path.isfile` on every unprocessed path); never bypasses when real raw remains. |
| **Checkpoint-complete async `DAY_CLOSE` at startup** | When all **mapped** closed raw for a calendar day is in **`.sync_timedb_state.json`** but large **`.tar`** / raw remain on disk, **`StartupDayClosePreflight`** (default **`sync_startup_day_close_preflight=yes`**) submits **`AsyncDayCloseCoordinator`** work on thread `startup-day-close-preflight`. Default **`sync_startup_drain_day_close_before_ingest=yes`** blocks first ingest until discover + async DAY_CLOSE + startup/day raw deletes idle. Submit budget applies **after** snapshot-derived scans. Manifest: **`{archive_data_dir}/.sync_timedb_startup_day_close.json`**. **Startup concurrency:** **`sync_startup_day_close_max_inflight`** defaults to **`archive_seal_parallel_workers`** (typically **4**); **`sync_startup_day_close_days_per_slice`** defaults to the same value. Steady-state janitor/immediate submit still uses **`sync_day_close_max_inflight`** (default **1**). Async worker pool size is **`max(sync_day_close_async_workers, sync_startup_day_close_max_inflight)`** so parallel seals can run during startup drain. Tunables: **`sync_startup_drain_day_close_before_ingest`**, **`sync_startup_day_close_budget_seconds`** (default **300**), optional **`sync_startup_day_close_scan_budget_seconds`** (warn-only), **`sync_day_close_async_workers`** (default **1**), **`sync_startup_day_close_backoff_seconds`** (default **30** when async pool saturated — skips expensive rescans). Candidate log: **`sync_day_close_candidate_report=yes`** (`queued` / `waiting_on_ingest` / `eligible_deferred` / `disqualified`; queued `async_in_progress` lines include **`async_last_progress`** / **`async_age_s`**). |
| **Async DAY_CLOSE raw-removal verify stall** | After seal, the async worker waits up to **`sync_day_close_raw_removal_wait_seconds`** (default **3600**; **`0`** = unbounded debug only) for **`DayRawRemovalCoordinator`** **verify** completion only, then sets manifest **`raw_delete_pending`** and releases the worker. Progress logs every **60s**; timeout → **`deferred`** / **`raw_removal_verify_timeout`**. Verified unlinks run on the **supervisor** at chunk boundaries (startup-parity brief gate): oldest-first per pass, **skip stuck older days for that batch** and continue younger days. Tunables: **`sync_day_close_raw_removal_max_deletes_per_pass`**, **`sync_day_close_raw_removal_verify_budget_seconds`**. Verify budget exhausted logs **`Day raw removal verify budget exhausted`**. Per-day manifests at **`phase=done`** with retryable skips reset for re-verify when closed raw remains. Restart recovery: **`sync_day_close_async_stale_seconds`** (default **7200**) marks stale **`submitted`/`sealing`/`raw_removal`** entries **`deferred`**. Grep: **`janitor: async day_close raw_removal verify stall`**, **`stale manifest recovery`**. |
...
| `sync_ingest_giant_pool_supplement_max_bytes` | 1073741824 | Max supplement path size (**1 GiB** exclusive) |
