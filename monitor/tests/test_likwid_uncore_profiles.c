...
static void test_eventset_nonempty(void)
{
  ...
  /* Regression: CHA eventsets must not reappear in uncore profiles. */
  assert(strstr(skx, "CBOX") == NULL);
  assert(strstr(icx, "LLC_LOOKUP") == NULL);
  assert(strstr(icx, "BYPASS_CHA") == NULL);
...
static void test_no_cha_profiles(void)
{
  /* 6 IMC + 4 DF; CHA enum slots must stay gone. */
  assert(LIKWID_UNCORE_PROFILE_COUNT == 10);
  assert(LIKWID_UNCORE_PROFILE_DF_ROME == 6);
}
