...
- **General**: Python 3.14+, `pip install -e ".[test]"`. Host workspace `.venv` must be GIL CPython 3.14 (Homebrew `python@3.14`). Compose image is `python:3.14.7-trixie` (GIL) for web; pipeline daemons run baked free-threaded `/opt/python3.14t` (`python-image-interpreter-contract.mdc`). Container `python3` is **not** the ingest interpreter. Image `hpcperfstats-base` source-builds numpy/numexpr/pandas against pip Intel MKL (both ABIs); that layer is longer than a wheel-only pip. Host unit tests: `hpcperfstats/tests/test_dockerfile_python_deps_cache.py`, `test_dockerfile_mkl_source_stack.py`, `test_pyproject_image_build_extra.py`. After an image rebuild, smoke `numpy.show_config()` on GIL and `/opt/python3.14t` (expect MKL). Do **not** install `.[image-build]` into the host `.venv` (no macOS ARM MKL wheels).
