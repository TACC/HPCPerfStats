...
1. **Throughput under skip-prewarm**: prioritize **`metrics_pool_process_cap`** (and worker CPU affinity / host sizing) and validating batched `Metrics.run` in production. References: `[DEFAULT] metrics_pool_process_cap` in `hpcperfstats.ini.example`, `hpcperfstats/analysis/metrics/metrics.py` (`Metrics.run`, pool + `_drain_metrics_imap`).
