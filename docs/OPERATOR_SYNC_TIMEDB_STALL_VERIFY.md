...
- **`status`** — in-process `current/queued`. **Always** includes `ingest_hot=0/0` / `ingest_catchup=0/0` when idle (do **not** grep `ingest_hot` alone as a stall probe — omit-zeros used to drop those tokens). Other kind ratios stay omit-zeros. Also `busy=` (incl discover), `orphan_inflight`, queue `*_q_delta`, `oldest_day` / `oldest_age_s`, optional **`fill_block=`** (dominant ingest fill failure: `claim_none`, `skip_missing`, `skip_reband`, `skip_lock`, `skip_fp`, `band_cap`, `submit_err`). Set when a fill submits 0 while **`len(local) < pool`** and the ingest score map is deep (under-capacity, not only local-empty).
- **`census`** — 60s depth; ratios are **inflight/queued**; `busy=` replaces opaque `local=I/A/D`. **Always** includes `total_ingested=` (process-lifetime successful `_drain_ingest_ready` `ingested` ACKs, including `0`; not db_skip) and `total_completed=` (process-lifetime successful `ingest_ok` ACKs including `db_skip` and other archive-complete outcomes). A **rising** `total_completed` across consecutive 60s lines means files completed even when `ingest=0/0` or `total_ingested` is frozen during db_skip catch-up. A **frozen** `total_completed` with `ingest=0/0` is idle ingest (not a dead reconstruct-coordinator). Supervisor restart resets both counters to 0.
- **Stuck-inflight (hpcperfstats02 2026-09-13):** `ingest=pool/deep` (pool **full**, e.g. `128/809x`) plus **frozen** `total_completed` (and frozen `total_ingested` when the work is not db_skip catch-up) is a **completion-rate** stall, not RC7–RC9 underfill. Pair with `parse_elapsed_s` hours, host CPUs **not** at `nproc`, thread `wchan=futex_do_wait`, and **`readlink /proc/<sync_timedb pid>/exe`**. PATH `python3` is GIL and is **not** the daemon. Sticky `discover=1/0` does **not** occupy ingest slots. Do **not** raise `sync_day_close_max_inflight`.
...
**Fail (T0 — discover stderr-pipe hang, hpcperfstats01 2026-09-17):** listend/RabbitMQ continue advancing while consecutive census lines stay `busy=discover`, `discover=0/1`, `ingest_hot=0/0`, and `total_ingested` is frozen; `boot discover submitted` exists but no matching `boot discover seen=`. `ps -T -p <main-pid>` shows the single `discover-bg` thread at 0% CPU in `pipe_read`; a full `py-spy dump --pid <main-pid>` shows that thread in `iter_find_stats_stdout_chunks` after stdout EOF. **Pass after non-pipe stderr + incremental-first boot:** `boot discover seen=` appears, the discover Future clears, current-window ingest advances before whole-archive catch-up, and no `discover-bg` thread remains stuck in `pipe_read`. Do not weaken H8 to permit concurrent walkers.

**Fail (T0 — discover stdout-pipe wait deadlock after TemporaryFile stderr, hpcperfstats01 2026-09-17):** fix is live (`TemporaryFile` present, not `stderr=PIPE`) but `discover-bg` sits in `do_wait` / `Popen.wait` while a child `stat` is `pipe_write` and `boot discover seen=` never lands. Root: generator `finally` called `wait()` after early close without closing/killing the still-writing stdout PIPE. **Pass after early-close kill/drain:** py-spy no longer shows `iter_find_stats_stdout_chunks` → `Popen.wait` beside a `stat` `pipe_write`; `boot discover seen=` appears when the walk finishes.

## Queue orchestrator era — remaining tiers

Production is **one** `sync_timedb.py` process per `archive_dir` (`run_sync_timedb_queue_orchestrator`, exclusive flock). Dual CLI ``backlog``/``current``, proximity heartbeat, `ArchiveJanitor` tick, handoff pins, and oldest-day **chunk gate** are **retired**. Operator census is **disk sidecars + thread titles** plus disk/DB reconstruct predicates — an empty `.sync_timedb_job_store.json` snapshot ≠ caught up. Pre-cutover T0 signatures below may mention Redis because that was the live job/members bus then; live census is the job-store sidecar and `.sync_timedb_archive_members/`.
...
**Fail (T0 — sticky ingest_hot=0/0 while RabbitMQ is up, hpcperfstats02 2026-09-14):** `queue_orchestrator status` shows sticky `ingest_hot=0/0` (empty hot ZSET, not `0/N` starvation) with `ingest_catchup` draining a deep map, RabbitMQ ready/consumers healthy, and listend idle lines advancing. Digit-epoch files exist under `{archive}/{host}/<unix>` while boot discover logs `seen=0` because catchup already exceeds `sync_ingest_queue_max_size` (find walk `break`) and/or ISO-only basename skip. **Do not** require `unresolved_day` in logs (count is 0 when classify never ran). **Not** RC1/RC7/RC8 (`ingest-refill-gap` / underfill / hot-post-RC7). **Pass after digit-epoch + hot-cap walk fix:** a new discover walk ZADDs today’s digit-epoch files into hot even while catchup stays above the member cap; do not expect an in-flight discover to retroactively enqueue. Do **not** raise `sync_ingest_queue_max_size`.

**Fail (T0 — retitled `[worker:populate-pool]` main / no ingest OS PIDs, hpcperfstats02 2026-09-13):** sole supervisord child argv is `sync_timedb.py [worker:populate-pool]` (PPID=supervisord, **large `nlwp`**, ~GiB RSS); no `[main]` and no `[worker:ingest-pool]` **process** rows; census `ingest=128/…` with `total_ingested` still rising; logs show `[sync_timedb:thread:ingest-coordinator]` / reconstruct. Root: populate **threads** called `setproctitle` and clobbered process argv; production ingest is an **in-process thread-pool** (occupancy ≠ OS worker count). Also greppable `queue_orchestrator ingest orphan reconcile failed: TypeError` when hygiene omitted `owner_token=`. **Pass after title + owner_token fix:** process argv stays `[main]`; thread titles remain `populate-pool` / ingest; no orphan-reconcile TypeError; do **not** treat sole populate title + large `nlwp` as a dead orphan to kill. Do not raise `sync_day_close_max_inflight`.

**Fail (T0 — stuck-inflight / ~1-core futex, hpcperfstats02 2026-09-13):** census `ingest=128/809x` (pool **full**) with `total_ingested` almost frozen; `parse_elapsed_s` hours ≫ `postgres_s`; host **~90% idle**; almost every ingest tid `wchan=futex_do_wait`. PATH `python3` GIL is **not** the daemon — require `readlink /proc/<pid>/exe` (`/opt/python3.14t/bin/python3.14t`). **Pass after persist_day lock fix:** ingest ACKs move while pool is full; persist copy does not serialize waiters; do not raise `sync_day_close_max_inflight`.

**Fail (T0 — clamp-era 40-inflight / 5–6 CPU underfill, hpcperfstats02 2026-09-13):** census `ingest=40/8xxx` with deep ZSET while INI `sync_ingest_pool_processes=128` and `nproc=40`; host ~5–6 CPUs; ingest **completing** (seconds–~10 min, not hour-scale); `/proc/<pid>/exe` is 3.14t. Root: occupancy clamp `min(INI pool, get_effective_cores())` pinned fill at nproc. Distinct from the 128/1-core futex stall. **Pass after clamp removal:** census `ingest=` equals INI pool (128 on this site) while ZSET is deep — **not** nproc; host CPU may still sit well below nproc (parse + Postgres + FT parking). Do **not** restore core clamp. Do **not** raise `sync_day_close_max_inflight`.

**Fail (T0 — remaining under-lock map-copy / ~1.5 CPU + futex after INI fill, hpcperfstats02 2026-09-13):** census `ingest=` equals INI pool (e.g. `128/…`) with `total_ingested` advancing; process ~1–2 CPU; almost all threads `wchan=futex_do_wait`; `parse_elapsed_s` ≫ `postgres_s`; empty non-idle `pg_stat_activity`. Distinct from stuck-inflight (frozen ACKs) and clamp-era underfill (`ingest=nproc`). Root: hot paths still held `SyncTimedbArchiveMembersStore` / job-store RLock across full `dict(members)` copies or disk persist. **Pass after map-copy lock fix:** lookup/wait/warm/job persist release the RLock before full-map copy or sidecar write; do **not** soft-cap fill below INI. Do **not** raise `sync_day_close_max_inflight`.

**Fail (T0 — remaining extra work under lock after map-copy, inventory 2026-09-13):** census `ingest=` equals INI pool with ACKs moving, but threads still serialize on RLock/SH across normalize/sort/parse CPU, `Event.set`, sealed populate `on_member`, or log `flush`. Distinct from map-copy (full `dict(members)` / persist copy) and clamp-era underfill. Root: remaining lock-hold inventory in members store, job store, streaming parse, sealed populate, and `log_print`. **Pass after lock-hold shrink:** those CPU/DB/`Event.set`/flush paths run outside the lock; maps replaced wholesale; `merge_members` CAS uses `get(key)` (never `or {}`); `claim_ingest` rechecks live score; parse keeps one fd. **Never** restore occupancy clamps (`min(INI, nproc)`). Do **not** raise `sync_day_close_max_inflight`.

**Fail (T0 — frozen-ACK / incremental `feed_line` under SH, hpcperfstats04 2026-09-13):** census `ingest=96/1208`→`96/1216` (pool **full** at site INI) with **`total_ingested` frozen** (e.g. `66`); process ~2–3 CPU / ~30 GiB RSS; almost all tids `wchan=futex_do_wait`; `/proc/<pid>/exe` is **`/opt/python3.14t/bin/python3.14t`**. Shrink fingerprint live (`_set_threading_events` present); `populate_wait|store_wait|Timed out waiting` grep **0**. `ingest file path=` is **bimodal**: parse-bound files (`parse_elapsed_s` hundreds–thousands, clusters ~412 / ~577 / ~688) vs postgres-bound (`postgres_s` hundreds–thousands with small parse). Root: `parse_stats_file_streaming_incremental` still ran `parser.feed_line` **inside** `_stats_file_read_lock` for the whole `STREAM_PARSE_LINE_BATCH` (streaming already batches readline under SH then `feed_lines` after release). Concurrent Timescale `COPY (SELECT * FROM _timescaledb_internal._hyper_*_chunk) TO STDOUT` explains the postgres-bound cohort only — **not** this product lock. **Pass after incremental SH shrink:** `feed_line` / sample-boundary emit run **outside** SH; ACKs move while `ingest=` equals INI fill; do **not** treat a 60s frozen counter alone as death when each file still takes minutes. **Never** restore occupancy clamps. Do **not** raise `sync_day_close_max_inflight`. Do **not** use awk `END{for (k in c)}` through podman-compose for wchan tallies (`uniq -c` instead).

**Fail (T0 — ACK-moving worker-futex under full INI fill, hpcperfstats01/02 2026-09-14):** census `ingest=` equals site INI pool with **`total_ingested` advancing** (not frozen-ACK); process still ~1–few CPU; **≥85%** non-jemalloc tids `wchan=futex_do_wait`. Operator finding #4: `futex_comm` tally shows **~112× `sync_timedb.py` / ~4× jemalloc / ~1× discover** — ingest workers dominate; jemalloc out of scope. Distinct from stuck-inflight (frozen ACKs), clamp-era underfill (`ingest=nproc`), and map-copy / feed_line SH shrinks already live. Root residual: populate `dequeue_populate` O(n) rank scan under `_populate_cv`; `load_stats_file_lines` full-file SH; `_should_stream_stats_file` size gate forcing non-stream; sealed populate `on_member` under SH. **Pass after lock-hold shrink wave (hot/cold deques + batch SH load + always-stream + sealed `defer_on_member`):** while ACKs move and `ingest=` equals INI fill, **non-jemalloc futex share &lt;50%** on a fresh wchan/`futex_comm` tally (exclude jemalloc TIDs). **Do not** treat py-spy dump failure on **3.14t** as undeployed. **Never** restore occupancy clamps (`min(INI, nproc)`). Do **not** raise `sync_day_close_max_inflight`.

**Ingest outcome-line telemetry (optional):** when INI **`sync_ingest_telemetry=yes`** (same CODE redeploy; **not** pipeline env — `no-production-env-for-ini-config.mdc`), `ingest file path=` lines add (1 decimal) parse-stage tokens **`lock_s`**, **`decode_s`**, **`feed_s`**, **`proc_merge_s`**, **`hw_df_s`**, **`proc_df_s`**, derived **`build_df_s`** (= merge+hw+proc frames), **`delta_s`**, **`collapse_s`**, **`arc_s`**, **`concat_s`**, **`start_s`**, **`stages_sum_s`**, closed-book **`parse_unaccounted_s`** = `parse_elapsed_s - stages_sum_s - postgres_s`, **and** write/COPY tokens **`orm_materialize_s`**, **`orm_bulk_prep_s`**, **`db_execute_s`**, **`db_commit_s`**, **`copy_s`** / **`conflict_insert_s`** (nested inside execute; not double-counted in residual). Lines always include **`host_insert_arm=`** / **`proc_insert_arm=`**. Default is **off**. Rename from retired `sync_ingest_parse_stage_telemetry` / `sync_ingest_write_telemetry`. Rank product cuts from the largest hold share; if `parse_unaccounted_s` stays large after `postgres_s` subtraction, instrument further — do not start another coarse telem wave. Mid-cohort pastes must use **named-field** regex. Pre-fix images can set telem yes and still **omit** write-phase tokens — treat missing `orm_materialize_s`/`db_execute_s` with telem on as undeployed log plumbing.

**Overnight decision soak (hpcperfstats04, post-COPY / post-E8):** after CODE redeploy with **`sync_ingest_telemetry=yes`** in site INI (same wave as E8 + single-INI telem getter — **not** compose `environment:`), soak ≥8 h continuous. Morning: capture full pipeline log with **`logs --timestamps`** (redirect `>file 2>&1`), run `python3 scripts/measure_pipeline_ingest_rate.py --log-file … --since-minutes 480`, read **`decision_next=`** plus mid-tier postgres frac / top parse hold / `mid_tier_parse_unaccounted_dominates` / `mid_tier_telem_incomplete`. Tokens: `telem_incomplete_re_soak` (parse holds present, write phases absent — fix INI/image, do not lock a hold); `parse_unaccounted_investigate` (residual ≥ top named hold — instrument before cutting a named hold); `parse_hold_<token>`; `write_timescale_path`; `stop_ingest_rate_watch_archive`. Do **not** treat flat files/s as “write unchanged” (loaded48 showed postgres_s collapse with flat files/s). Do **not** raise ingest pool. Do **not** enable telem via compose `environment:`.

**E8 / E9 post-redeploy T0 (delta/collapse CODE):** with **`sync_ingest_telemetry=yes`**, confirm mid-file **`delta_s=`** / **`collapse_s=`** are lower than the 2026-09-26 overnight cohort (~680–780 / ~540–580 on 13–17 MiB) and that campaign artifact `e8_delta_collapse_ab_*.json` recorded **`retain_delta_s`** and **`retain_collapse_s`**. files/s alone is not the retain meter. Then lock next CODE from the ≥8 h `decision_next` (campaign **E9**, plan `04-post-e8-overnight`).
...
**E7 residual merge/build_df (campaign 2026-09-19):** local compose paired A/B at ingest width **48** (`--e7`) tried skip-redundant builder peak-merge + columnar proc flush after Wave4 online merge; **retain=false** (`e7_proc_build_ab_da062d9bdc6f45ce8a1a11f95e02ebb6.json`) — product reverted. Expect production still to show residual `proc_merge_s` / derived `build_df_s` on closed-book telem; that is not a stall by itself and is not fixed by raising ingest pool. Re-attack only with a new CI win or larger-cohort/flamegraph evidence (see `docs/SYNC_TIMEDB_THROUGHPUT_CAMPAIGN.md`).
...
**Fail (T0 — merge-then-yield / H17, hpcperfstats02 2026-09-03):** `stage_enter reconcile_merge` → `ok` → `pre_seal_verify ok` → `wait_on_ingest yielded` on the same claim (merge write-lock before cooperative yield). Some days skip `wait_on_ingest`, run `dedupe`/`seal`/`raw_delete`, then `vacate outcome=incomplete_raw` when `should_handoff_to_ingest` is false but closed raw remains (`has_closed_raw_on_disk` broader than verify-handoff). Giants rewrite on every reclaim; no `complete=` / `tar_delete=` on age-eligible no-raw days. **Pass after H17:** remaining-raw days show `wait_on_ingest` **without** preceding `reconcile_merge` on that claim; `incomplete_raw=` or `yielded=` without merge-per-yield loop; age-eligible no-remaining-raw days reach `complete=` / `tar_delete=`.

**Fail (T0 — post-H17 yield spin / idle handoff, hpcperfstats04 2026-09-03):** continuous `day_close claim` → `vacate … outcome=yielded` for many open days lasting **minutes**, with **append LLEN=0** and **ingest ZCARD=0** (not backlog waiting). Root: cheap H17 yield + immediate requeue + ~50 ms coordinator sleep, and `complete_handoff_to_ingest` no-op when verify-handoff is false. **Pass after yield-spin fix:** claim/vacate `yielded` lines are **rate-limited** (`suppressed_n=`); coordinator sleeps ~`sync_pool_poll_timeout_s` on yield/backoff-only churn; sticky per-day yield reclaim backoff (30s→300s); closed-raw **kick** refills ingest/append while days still yield; 600s `progress day=` shows `yielded=` without a claim/vacate firehose.

**Fail (T0 — post-H17 open-tar forever-yield / H18, hpcperfstats02 2026-09-04):** H17 + yield-spin fingerprints live; ingest/append idle; `day_close` LIST deep; progress is almost only `yielded=` / `dc_run=` with **no** `complete=` / `tar_delete=` on age-eligible open days; kick/handoff/`suppressed_n=` greps near **0**. Root: `kick_closed_raw_unblock` returned pure **`noop`** for retryable-only / empty-handoff closed raw (yield never drove ingest/delete), and FIFO day_close fill starved frozen older days behind growing newest giants under `sync_day_close_max_inflight`. **Pass after H18:** yield still before merge; kick returns **`handoff`** / **`delete_reopen`** / **`verify`** (not noop) when `has_closed_raw_on_disk`; greppable closed-raw handoff/kick lines; oldest calendar day_close claims preferred; age-eligible no-remaining-raw days eventually show `complete=` / `tar_delete=`.

**Fail (T0 — post-H18 wait_on_ingest forever-yield / H19, hpcperfstats02 2026-09-06):** H18 fingerprints live (`h18_handoff_retryable`, `h18_has_closed_fallback`, `h18_oldest_fill`, yield-spin backoff); job-store **`append_n=0`**; `day_close` LIST deep; claim → `stage=wait_on_ingest` → `vacate outcome=yielded` cycles for age-eligible open days stuck **`phase=verifying`** with `has_closed True`. Ops `kick_closed_raw_unblock` returns **`verify`** (productive) but the worker path never reaches verify/delete. Root: H17 `wait_on_ingest` yielded whenever `has_closed_raw_on_disk` even when append was idle and ingest/populate was not hot. **Pass after H19:** idle append + no hot does **not** forever-yield; expect `wait_on_ingest skip_yield reason=append_idle`, closed-raw **verify** kick, `apply_batch_delete` / `incomplete_raw=` progress, and no `reconcile_merge` on that idle remaining-raw claim. Append-queued or ingest-hot days still yield before merge (H17).

**Fail (T0 — post-H19 false-hot forever-yield / H20a, hpcperfstats02 2026-09-07):** H19 fingerprints live (`_day_close_append_or_hot_active`, `skip_yield`+`append_idle`, `kick_closed_raw_unblock`); job-store **`append_n=0`**; unbroken `vacate outcome=yielded` with **zero** `wait_on_ingest skip_yield`. Root: leftover or kick-set ``ingest_tar_hot`` (and complete members maps via ``day_close_yield_requested``) made wait_on_ingest treat idle days as hot; kick-then-hot measured occupancy after handoff. **Pass after H20a:** leftover / kick-set ``ingest_tar_hot`` skip-yields; expect `wait_on_ingest skip_yield reason=append_idle` on age-eligible idle days; live append LIST, live populate owner, or ingest jobs for **that** day still yield before merge (H17 / 09-05 class).

**Fail (T0 — post-H20a skip_yield then incomplete_raw / H20b, hpcperfstats02 2026-09-08):** H20a skip_yield is live; frozen age-eligible days (`2026-08-17`, `08-28`) still `vacate outcome=incomplete_raw` after `wait_on_ingest skip_yield reason=append_idle` then `skip_merge … remaining_raw`. Census: **`phase=deleting`**, `has_closed=True`, `handoff=False`; **no** `tar_drop`. Root: `needs_ghost_delete_retry` required `delete_phase_done()` (`phase=done` only), so `deleted=True` files on disk were skipped by `apply_batch_delete` (deleted=0). **Pass after H20b:** ghost retry also runs in `phase=deleting`; expect `apply_batch_delete` unlinks remaining closed raw, then `tar_delete=` / `complete=` on age-eligible no-remaining-raw days. Do not require `phase=done` for ghost retry. Growing `08-31`–today remains H17 soak. `2022-11-05/06` yielded and ghost `2023-05-08` identity are out of H20b scope.

**Fail (T0 — post-H20b skip_yield remaining_raw / H20c, hpcperfstats02 2026-09-08):** H20b is live (`needs_ghost_delete_retry` allows `PHASE_DELETING`); frozen `08-17`/`08-28`/`08-23`/`08-25` still `phase=deleting`, `needs_ghost=0`, `verified_pending=0`, `only_waiting=0`, `blocking_n = closed_raw_n = remaining_raw_n`. Skip_yield `raw_delete` unlinks only `status=verified`; reclassify sourced retryable skips only, so blocking non-retryable remaining returned 0 and `deleted=0`. **Pass after H20c:** when `verified_pending=0`, reclassify **blocking** paths under the DB ingest-ready gate, then unlink; frozen Aug days must eventually `tar_drop` / `complete=` when no remaining raw. Growing `08-31`–today still H17. Do not require remaining_raw orchestrator refresh.

**Fail (T0 — post-H20c complete-map populate_active yield / H20d, hpcperfstats02 2026-09-09):** H20c reclassify is live; frozen `2022-11-05` / `2022-11-06` census remaining/blocking/closed_raw/only_waiting/needs_ghost/has_closed **0**, tar+zst **1**, `phase=verification_complete`. Logs: claim → `pre_seal_verify` ok → `reconcile_merge` → **`yielded reason=populate_active`** → vacate, with **no** live populate owner. Root: merge yield used `archive_members_populate_shows_progress_for_day` (True when the members map is complete), while job-entry used owner-only. **Pass after H20d:** `populate_active` yield only when `archive_members_populate_owner_active_for_day`; remaining=0 days reach `tar_drop` / `complete=` after classify. Do **not** skip `reconcile_merge` solely because `remaining_raw==0` (sibling-tar exemption). `2026-08-28` 187G in-flight GNU tvf scan is residual soak, not this bug. Growing `08-31`–today still H17.

**Fail (T0 — reconstruct skip of day_close refill / H21, hpcperfstats04 2026-09-10):** reconstruct-coordinator census is always `busy=discover`; age-eligible open tars exist on disk but are **not** on the day_close LIST; dead-letter sidecar absent; inflight=0. Root: `_idle_reconstruct_pass` returned 0 when `_discover_bg_is_busy()` and skipped `_enqueue_day_closes_for_daily_dir`. **Pass after H21:** reconstruct still must not submit discover while discover-bg is busy; cheap daily-dir enqueue still runs; leftover/frozen age-eligible tars appear on LIST (`reconstruct_enq=` / `incomplete_seen=`). Remaining>0 verifying yield-spin days stay H17 soak. Do not skip `reconcile_merge` because remaining_raw==0.

**Fail (T0 — leftover inflight hang before first stage_enter / H22, hpcperfstats04 2026-09-11):** H21 live; leftover `07-28`/`08-03`/`08-04`/`08-09` occupy all `sync_day_close_max_inflight` slots (claim identities, persist-omits-inflight LIST holes); **empty** `stage_` / `vacate` / `janitor: day_close` in the same log buffer. Root: `_maybe_yield_disk_remaining_raw` called `has_closed_raw_on_disk` → `_build_remaining_raw_for_daily_tar` **before** `_stage_enter("pre_seal_verify")`. **Pass after H22:** leftover claims emit `stage_enter disk_remaining_raw` before the yield probe; job-entry `needs_wait` uses cheap manifest `phase` (`verifying`/`deleting`, or `verification_complete` with cheap verified-pending) not remaining-raw find. Remaining=0 `verification_complete` still `reconcile_merge` (H20d). Do not treat empty stage/vacate after claims as expected verify soak. Do not raise inflight as the leftover fix.

**Fail (T0 — leftover hang before stage_enter with H24 deployed / H22 get_day prefix, hpcperfstats04 2026-09-24):** H24+H22 source tokens live in installed `qo.__file__`; sticky `day_close=` at `sync_day_close_max_inflight` with **claim|vacate|stage_enter count=0** (claim INFO may age out / rate-limit; **stage_enter is decisive**) and dual sticky days in `deleting`/`verification_complete` with `deleted_count=0`. Root: `_get_or_create_day` / `_begin_closed_raw_pass_memo` (manifest load) ran **before** `_stage_enter("disk_remaining_raw")`, so hang left empty stage breadcrumbs. **Pass after fix:** open-tar claims emit `stage_enter disk_remaining_raw` **before** get_day/begin_memo; expect stage_enter lines while leftover days remain claimed. Do not treat empty stage with H24 True as undeployed-H22. Do not raise inflight.

**Fail (T0 — leftover hang after stage_enter disk_remaining_raw / H23, hpcperfstats04 2026-09-11):** H22 live (`disk_remaining_raw` in installed `qo.__file__`); leftover `07-28`/`08-03`/`08-04`/`08-09` `claim` then `stage_enter … stage=disk_remaining_raw` with **no** `vacate` / `wait_on_ingest` in the same grep. Oscillating `day_close=` at cap 4. Root: cheap `phase=verifying` (or `verification_complete`+pending) already answers `needs_wait`, then `_day_close_complete_wait_on_ingest_handoff` still calls `kick_closed_raw_paths_to_ingest` → remaining-raw find. **Pass after H23:** leftover verifying days skip that remaining-raw kick, still `kick_closed_raw_unblock`, then yield/`skip_yield`/`vacate` without occupying inflight on that walk. H17 idle remaining-raw kick still runs when cheap phase does **not** answer. Do not skip `reconcile_merge` because remaining_raw==0. Do not raise inflight.

**Fail (T0 — leftover hang after stage_enter disk_remaining_raw / H24, hpcperfstats04 2026-09-12):** H23 live (remaining-raw kick skipped); leftover `08-03`/`08-09` `phase=verification_complete` with verified-pending 799/826 occupy inflight after `stage_enter disk_remaining_raw` without `pre_seal_verify`. Root: `_day_close_complete_wait_on_ingest_handoff` still called `complete_handoff_to_ingest` → remaining-raw walk via `should_handoff_day_close_to_ingest`. Leftover `07-28`/`08-04` `phase=verifying` soak in GNU `pre_seal_verify` (not this hang). **Pass after H24:** skip `complete_handoff_to_ingest` when cheap already answers `needs_wait`; still `kick_closed_raw_unblock`; H17 idle remaining-raw kick still runs when cheap does not answer. Do not skip `reconcile_merge` because remaining_raw==0. Do not raise inflight.

**Fail (T0/T1 — seal hang after H19 skip_merge, hpcperfstats01 2026-09-25):** age-eligible giant `2026-07-28` (dual tar+zst, ~749G) claims a `sync_day_close_max_inflight` slot; logs show `wait_on_ingest skip_yield reason=append_idle` → `skip_merge … append_idle_remaining_raw` → `stage_enter seal` with **no** `stage_exit seal` / **no** `tar_drop` / open_n flat across T0→T1. Root: after H19 skip_merge, seal entry still called `remaining_raw_paths_blocking_tar_drop` (full remaining-raw find) before `seal_dirty_daily_archives`. Installed image may also miss enter-before-get_day (`h22_enter_before_get_day False`) — redeploy that with the seal fix. **Pass after fix:** when `skip_merge_remaining_raw` is set, seal passes a cheap non-empty `remaining_raw_by_gz` sentinel (no remaining_fn call) so `only_when_no_remaining_raw` still no-ops compress while raw remains; post-delete reseal may use empty map; expect `stage_exit seal` and eventual `tar_drop` / open_n decline on age-eligible dual days. Do **not** raise inflight. Do **not** treat calendar current-day `.tar` as failure. Do **not** pass empty `{}` at first seal after skip_merge (that would incorrectly allow seal while remaining raw exists).
...
**After absolute INI pool-size redeploy (`sync_ingest_pool_processes` / `metrics_pool_processes` / `gunicorn_workers` / `listend_db_ingest_pool_processes`, 2026-09):** this is a **pool sizing** change, not a stall-campaign fix. Post-redeploy use **T0/T1 only** — confirm ingest still advances (`chunk_elapsed_s`, `ingest file path=… outcome=…`, active ingest threads ≤ `sync_ingest_pool_processes`). Do **not** run a full **T2** multi-hour stall campaign solely for the absolute-INI cutover unless a new stall signature appears. Secondary caps/budget/overlap/`metrics_prewarm_*` keys are gone; stale site INI lines for those keys are ignored.

**After the metrics in-process cutover (2026-09-03):** T0 after redeploy of `pipeline` must show one free-threaded `update_metrics.py [main]` process, titled `[thread:public-ef-pool]` during `/pub` work and `[thread:metrics-pool]` during job metrics. There must be no `[worker:metrics-pool]` / `[worker:public-ef-pool]` PIDs and no forkserver control process under metrics main. `metrics_pool_processes` is now the thread count; stale `metrics_pool_maxtasksperchild` and `metrics_run_per_job_timeout_s` INI lines are ignored. This cutover alone requires T0 only; run T1/T2 when ingest/day-close behavior changes or a new stall signature appears.
...
**After dual-zstd / exclusive restore deploy:** at most one `archive: daily_tar_restore begin` per calendar day until matching owner `end`; never two `zstd -d -o …decomp.tmp` for the same day. Classify: `zstd -d -c` = members-store populate (OK); `zstd -d -o …decomp.tmp` = sealed→tar restore (must be single-flight). On redeploy, stop pipeline, remove stale `*.tar.decomp.tmp` under INI `daily_archive_dir`, then up — see **T0 / T1 — exclusive daily tar restore (dual-zstd)** below.
...
| **T1 progress (`current`)** | Same window while CLI ``current`` (newest-first) runs | Descending `epochs=` on `chunk dispatch begin`; `youngest_day_chunk_gate` / `youngest_day_chunk_gate_pad` (not ascending oldest-gate for ``backlog``); heartbeat sidecar advances with active work |
...
**Pass (T1):** under combined ingest with a warm July members store + concurrent June populate, ingest continues without exit 124 on the store_warm small-window path; `chunk ingest summary` cadence resumes after large-file / member-wait windows.
...
### T0 / T1 / T2 — open-tar vs store membership divergence (hpcperfstats03, 2026-08-23)
...
**Signature (pre-fix):** all open daily `.tar` mtimes stale >3d (`tars_mtime_older_than_3d` equals `tar_count`); **`Archived batch=0`**; **`archive_job_duty` with `to_add=0`** on every job (e.g. 47/47); newest calendar days (e.g. **2026-08-20+**) have **no** `.tar`; June days occupy oldest-first archive slots with **`mapped>0`** but **`appended=0`**. Often coexists with open-tar/store divergence on older mutable tars and skip-invalidate thrash when pin-hold is not deployed.
...
**Pass (T0):** at least one sticky day logs **`to_add>0`** / **`appended>0`** or **`Archived batch`**; newest calendar days gain `.tar` files or advancing mtimes; **`members_source=tar_scan`** on mutable-tar jobs (see open-tar/store divergence section).
...
**Healthy (post-fix):** membership from store/sealed **before** decompress. When candidates are already members:
...
**Archive signal:** prefer `archive_job_begin` / `archive_job_done` / `Archived batch` / `Archive mapping` — **not** a bare `tar` grep (misses store-noop days and worker-role lines).
...
**Pass (T1 — empty store after prewarm):** After `chunk prewarm complete` with a day token other than `no_daily_archive` / `day_ingest_skip`, the members store must be warm and **`chunk imap start`** must appear. **Failure signature (pre-fix, claimed success hang):** `…:prewarmed` (or similar) with an empty store and no `chunk_elapsed` / no `chunk imap start`. **Post-fix (true empty):** supervisor **exits** with `archive members store empty after prewarm` rather than hanging.

**T0 / T1 — prewarm identity drift during tar append (2026-07):** concurrent `archive_pool` append/merge can change the store identity fingerprint while chunk prewarm holds entry-time keys.

```bash
# T0 — false empty-after-prewarm during append (unhealthy)
grep -E 'populate_wait identity_drift|empty after prewarm|archive members store empty after prewarm|tar_append store merge|archive_job_done' /tmp/pipeline-full.log | tail -80

# T1 — healthy: identity_drift may appear, but prewarm re-resolves / retries; no empty-store exit
...
**Unhealthy:** `INFO: populate_wait identity_drift day=…` then `ERROR: archive members store empty after prewarm … source=none members_n=N` (N often >0) while the same day shows `tar_append store merge` / `archive_job_done`. **Healthy:** drift and/or `WARNING: archive_append_inflight during archive members prewarm … retrying` then `chunk prewarm complete` with `store_warm` / populate source and **`chunk imap start`** — no empty-after-prewarm exit.

**T0 / T1 — L1 hit + cold store (no append_inflight) (2026-07-21):** process L1 can return `members_n>0` with `source=none` while the members store stays cold; prewarm must **not** exit on the first cold check when retries remain.

```bash
# T0 — false empty-after-prewarm from L1 / members_n>0 without inflight (unhealthy)
grep -E 'members returned but store cold|empty after prewarm|archive members store empty after prewarm|Prewarming archive members|chunk prewarm begin|chunk imap start' /tmp/pipeline-full.log | tail -80

# T1 — healthy: WARNING members returned but store cold … retrying then warm / imap start
grep -E 'members returned but store cold|archive_append_inflight during archive members prewarm|chunk prewarm complete|chunk imap start|empty after prewarm' /tmp/pipeline-full.log | tail -80
```

**Unhealthy:** `Prewarming …` then within ~10ms `ERROR: … empty after prewarm … source=none members_n=N` with **no** `members returned but store cold` / inflight WARN (often tar-only day, store `hlen=0`). **Healthy:** `WARNING: members returned but store cold … retrying` (and/or inflight WARN) then prewarm success and **`chunk imap start`** — no immediate empty-store exit.
...
- Then **`Archive members populate stalled (no progress for 120s)`** → supervisor **`exit status 1`** while the members store is still reachable.
...
- Stall defer prefers **`store_populate_active`** over **`idle_pool_ghost_inflight`** while the members-store populate lock is held.
