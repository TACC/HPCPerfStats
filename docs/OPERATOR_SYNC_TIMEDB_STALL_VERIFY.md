...
**Wave 3/4 parse-stage shares (optional):** when INI **`sync_ingest_parse_stage_telemetry=yes`**, `ingest file path=` lines add **`feed_s=`** / **`collapse_s=`** / **`build_df_s=`** (same 1-decimal shape as `postgres_s`). Default is **off** (no clocks). Use stage shares vs `parse_elapsed_s` to rank emit vs collapse vs flush for Wave 4 — **not** a soft-kill budget. Mid-cohort pastes must use **named-field** regex (do not require `size_bytes` before `elapsed_s` on the line).
