...
Stay on the **2.28.x** line while on PostgreSQL 15 — Timescale **2.29+ drops PG15**. Do not change the compose tag to a newer major Timescale without a PG major upgrade plan. Homemade PG18 + Timescale 2.30.x dual-run and logical cutover: **`docs/OPERATOR_PG18_MIGRATION.md`** (do **not** `pg_upgrade` or restore `_timescaledb_catalog` from PG15).
