...
**`scripts/rebuild_pipeline.sh`** rebuilds the shared **`hpcperfstats`** image for Python/pipeline code changes: it preserves the live **`STATIC_ROOT/frontend`** tree, stops **`pipeline`** then **`web`**, builds Docker target **`hpcperfstats-pipeline-refresh`** (skips **`frontend-builder`** / npm), and recreates **`web`** then **`pipeline`** (pipeline is always brought up last — including via the EXIT trap if web recreate is interrupted; podman web recreate removes **`web` only**, not **`pipeline`**; both use **`--detach --no-deps`** and wipe **`*_tmp*`** name collisions). On exit it removes host scratch **`.build/pipeline-rebuild-frontend/`** (and **`rmdir`s `.build` when empty**; sibling monitor/other `.build` contents are left intact). **`--no-web`** is a temporary pipeline-only path (no running **`web`** required): seeds SPA from the existing image or placeholders, recreates **`pipeline`** only, and warns that a proper rebuild (without **`--no-web`**) is required before bringing **`web`** / the full stack back. On **podman-compose** hosts it uses **`podman build --target`** directly because **`podman-compose build`** does not accept **`--target`**, and after stop+build it removes stale **`web`/`pipeline`** containers with **`podman rm -f`** (via **`compose_podman_rm_service_containers`** — **not** ``docker compose rm``, which podman-compose rejects) before **`compose up --detach --no-deps`**, briefly stopping **`proxy`** when it was running so fixed names like **`hpcperfstats_web_1`** can be reused. On Docker/Colima it uses **`compose up --detach --force-recreate --no-deps`** instead. For a fire-and-forget recreate **without** rebuilding the image, use **`./scripts/recreate_web_pipeline.sh`** (static: **`./scripts/test_recreate_web_pipeline.sh`**). It does **not** replace compose test workflows; those still use **`--skip-build`** when reusing an existing image. Static regression: **`./scripts/test_rebuild_pipeline.sh`** and **`./scripts/test_rebuild_frontend_verify.sh`**.
...
**Nginx static + SPA-shell contract (WSGI):** In production, nginx serves both **`/static/*`** and the SPA shells for **`/machine/*`** and **`/pub/*`**; the **`proxy`** image is **source-built** (**nginx 1.31.x** + jemalloc/zlib-ng/OpenSSL/ngx_brotli/static zstd; see **`test_proxy_dockerfile_source_builds_nginx_with_pinned_deps`**) and **`include`**s **`hps-proxy-allowed-hosts.inc`** generated from **`[DEFAULT] server=`** via **`services-conf/parse_hpcperfstats_proxy_hosts.py`** and **`write_nginx_proxy_allowed_hosts_include.py`** (see **`hpcperfstats/tests/test_write_nginx_proxy_allowed_hosts_include.py`**), forwards only an explicit allowlist of paths to Gunicorn, and returns **404** for unknown paths at the edge. **`docker-compose.yaml`** bind-mounts **`./services-conf/nginx-main.conf`** onto **`/etc/nginx/nginx.conf`**, committed **`./services-conf/nginx.conf`** onto **`default.conf`** for **`proxy`**, mounts **`proxy_ssl_source:/mnt/ssl-source:ro`**, and bind-mounts shared snippets (**`nginx-static-files.conf`**, edge/CSP/proxy-common, and hybrid **`nginx-compress-proxy.inc`** / **`nginx-compress-static.inc`**) as the **only** runtime source for those snippets (**`proxy.Dockerfile`** does not **`COPY`** them). Host pytest **`hpcperfstats/site/hpcperfstats_site/tests/test_compress_static_sidecars.py`** locks Python **`.br`/`.gz`** sidecar generation after collectstatic/heal (`copy-next-export` does not compress). SPA **`/machine/`** and **`/pub/`** set **`open_file_cache off`**. TLS PEMs are **materialized at container start** from the settings mount via **`resolve_proxy_ssl_certs_dir.py`** in **`proxy_entrypoint.sh`** (fixed nginx paths under **`/etc/ssl/hpcperfstats/`**; no build-time bake, no **`additional_contexts`**, no **`.env`** / **`HPCPERFSTATS_SSL_CERTS_DIR`**; test overlay binds **`test_proxy_ssl_source`** to **`./tests/fixtures/proxy-ssl`**). Edge HSTS/framing/COOP/Permissions-Policy/Referrer-Policy live in **`nginx-edge-security-headers.inc`**; SPA CSP hashes are emitted by **`copy-next-export.mjs`** into **`nginx-csp-machine.inc`** / **`nginx-csp-pub.inc`** on the static volume; OCSP resolver generation is covered by **`hpcperfstats/tests/test_write_nginx_resolver_include.py`**. Gunicorn/Django must still not answer **`STATIC_URL`** or SPA shell routes directly. Host pytest includes **`hpcperfstats/site/hpcperfstats_site/tests/test_nginx_static_wsgi_contract.py`**, which asserts the Django **`Client`** (WSGI) does not resolve arbitrary **`STATIC_URL`** paths or SPA shell routes and snapshots the nginx allowlist snippets. For **local dev** matching that split, prefer compose with **`proxy`** or **`manage.py runserver --nostatic`** plus nginx for **`/static/`** and `/machine/*` shell delivery (see **`README.md`** and **`hpcperfstats/cursor-rules/nginx-static-url-prefix.mdc`**).
...
**Thread reset / prewarm stall:** `MetricsPrewarmStallError` at partial `completed=k/n` must keep partial successes, soft-fail the rest, detach the current executor, and lazily create a fresh titled pool before the next batch (do not wipe results to `[]`). `test_update_metrics_architecture_contract.py` rejects metrics process pools, process-only timers/reaping, and retired metrics recycle INI keys, and locks `configure_blas_thread_env()` before Django/metrics numpy imports.

**Slow batch vs hang:** `inflight_jids ≈ ready_dequeued_total` with **`compute_batches_total=0`** / **`processed_total=0`** for a long window may be a **slow first compute batch** (metrics and/or **`pipeline_required`** prewarm under DB load), not a dead pool. Confirm **`[thread:metrics-pool]`** titles under `update_metrics.py [main]`, watch for `compute batch heartbeat` / rising **`compute_batch_completed_jids`**, then wait for `compute batch N … compute_batch_elapsed_s=… next_batch_cap=…`. **`metrics_compute_watchdog_s`** downshifts **`next_batch_cap`** from **metrics+prewarm `batch_wall_s`** (default total-wall INI `metrics_compute_total_watchdog_s=0` is not required for that downshift).
...
- **`no_ready_candidates`**: starvation at readiness/candidate stage (no ready enqueue / mid-batch / inflight progress for ``STALL_EXIT_AFTER_SECONDS``). Must **not** fire while ``ready_queue`` is non-empty, ``inflight_jids > 0``, or mid-batch ``compute_batch_completed_jids`` / ``ready_enqueued_total`` is still advancing — a long first metrics+prewarm batch with rising heartbeat ``completed_jids`` is a slow batch, not this stall.
...
**Candidate listing SQL (three-phase):** Phase A ``_jobs_queryset`` keyset pages must **omit** correlated ``live_distinct`` ``host_data`` aggregates and plot/detail ``encode(sha256…)`` fingerprints (cheap ``md_count`` / ``stale_null`` / gate-failure only). Phase A2 ``_jobs_queryset_live_distinct_catchup`` + ``_page_rows_needing_live_distinct_refresh`` refreshes live-stale jobs via page-bounded ``get_live_distinct_time_count_for_jid``. Phase B ``_jobs_queryset_artifact_catchup`` + ``_page_rows_needing_artifact_refresh`` lists artifact-only jobs via cheap existence then bounded page FP checks. Empty-pass ``listed=`` census uses cheap Phase A only (not live-refresh backlog). Host unit coverage: ``test_jobs_queryset_listing_sql_omits_live_distinct_and_sha256``, ``test_page_rows_needing_live_distinct_refresh_keeps_stale``, ``test_jobs_queryset_still_lists_artifact_only_via_phase_b``, ``test_census_listed_avoids_live_distinct_and_fingerprint_sql``, ``test_sliding_supplements_invoke_on_supplements_taken`` in ``hpcperfstats/site/lib/machine/tests/test_update_metrics.py``.
...
| 2 | `tests/run_redis_cache_pytest_workflow.sh --skip-build` | Fresh compose session, Django **`test_redis_cache_live.py`** against real **Redis** (`HPCPERFSTATS_PYTEST_LIVE_REDIS=1`). `sync_timedb` no longer uses Redis for job queues or member maps. |
...
| `hpcperfstats/tests/test_sync_timedb_archive_members_coord.py` | In-process member-store populate + wait (`test_populate_store_single_flight_and_warm_lookup`, `test_sticky_skip_survives_reload`). Renamed with the store cutover. |
| `hpcperfstats/tests/test_sync_timedb_job_queue.py` | In-process job-store reconstruct/discover/streaming helpers plus leftover host claim tests. Operator census must use `.sync_timedb_job_store.json` and `[thread:populate-pool]` (`test_operator_census_uses_disk_sidecars_and_thread_titles`). Django cache live Redis remains `test_redis_cache_live.py`. |
| `hpcperfstats/tests/test_sync_timedb_job_discover.py` | Streaming discover calendar-day resolver, ingest queue cap stop, mtime window, CLI date-range filter. |
...
Logs to **`test_runs/day-close-loop-regression-battery-<timestamp>.log`**. After the in-process cutover, covers job-store reconstruct / discover helpers (`test_sync_timedb_job_queue.py`, `test_sync_timedb_job_discover.py`, including digit-epoch calendar-day and over-cap hot ZADD via `-k` tokens `digit_epoch` / `over_cap` / `calendar_day`), exclusive flock + entry wiring (`test_sync_timedb_queue_orchestrator.py`), append lock/`tar -r` (`test_sync_timedb_append_tar_race.py`), subprocess drain/timeout (`test_sync_timedb_subprocess_hardening.py`), member-store smoke (`test_sync_timedb_archive_members_coord.py`), B-09 predicate `test_arch_*` (`test_sync_timedb_architecture_contract.py`), fd `-X` GNU stat discover, day-raw-removal samples (including ghost-deleted retry), day-close hot-path `populate_active` owner vs complete-map (`test_sync_timedb_day_close_cooperation.py`), gz-only migrate nested lock (`already_locked` / `nested_lock` in `test_sync_timedb_archive.py`), and **`--jid` ingest-only** (`test_sync_timedb_jid.py`). Retired supervisor/janitor two-queue tests are no longer in this battery. See **`sync-timedb-change-regression-gate.mdc`**, **`sync-timedb-queue-orchestrator-contract.mdc`**, and **`docs/OPERATOR_SYNC_TIMEDB_STALL_VERIFY.md`** (**Queue orchestrator era** T0/T1/T2; `--jid` uses the smoke note there).
