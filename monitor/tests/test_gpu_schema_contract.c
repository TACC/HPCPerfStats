...
int main(void)
{
  ...
  assert(strstr(nvidia_schema_def, " gpu_pcie_tx_bytes,E,W=64,U=B") != NULL);
  assert(strstr(nvidia_schema_def, " gpu_pcie_rx_bytes,E,W=64,U=B") != NULL);
  assert(strstr(nvidia_schema_def, " gpu_nvlink_tx_bytes,E,W=64,U=B") != NULL);
  assert(strstr(nvidia_schema_def, " gpu_nvlink_rx_bytes,E,W=64,U=B") != NULL);
  assert(strstr(nvidia_schema_def, " gpu_pcie_replay_counter,E,W=64") != NULL);
  assert(strstr(nvidia_schema_def, " gpu_io_link_total_bytes,E,W=64,U=B") != NULL);
