...
| `hpcperfstats/tests/test_sync_timedb_job_queue.py` | Greenfield orchestrator **job:v1** helpers (slices 1–3, unused by `sync_timedb.py` main): key names, hot/catchup score encode/decode, reserved slot caps **10/6** at pool 16, `SET NX EX` leases + dead-owner steal, ranged Lua pop via `EVALSHA` (no whole-key `ZPOPMIN`), LIST FIFO for append/discover/day_close, invalidate deny-list protects `job:v1`; brownfield **reconstruct** classify/enqueue; **streaming discover** (find `-printf` chunks → `ZADD` before iterator exhaust; skip already-complete) (host `FakeRedis`). |
