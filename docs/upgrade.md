...
Compose **`mem_limit` / `memswap_limit` 96g** plus `services-conf/rabbitmq_vm_memory.conf` (`vm_memory_high_watermark.absolute = 80GiB` headroom). Compose **`ERL_FLAGS=+MBas aobf +MBlmbcs 512 +MHlmbcs 512`** tunes Erlang binary/heap carriers (recreate after changing that env). Console logging is **`warning`** with connection/channel **`error`** (`services-conf/rabbitmq_logging.conf` — not **`info`**). Crash dumps off: **`ERL_CRASH_DUMP_SECONDS=0`** + **`ulimits.core: 0`**. Recreate `rabbitmq` after changing mem limits, watermark, **`ERL_FLAGS`**, logging, crash-dump knobs, or **`services-conf/rabbitmq_frame_max.conf`** (`frame_max = 131072`; `docker compose up -d --force-recreate rabbitmq pipeline`). On hosts with less than 96 GiB RAM, lower **both** `mem_limit`/`memswap_limit` and the absolute watermark together. Inspect: `docker compose exec rabbitmq rabbitmqctl status` (Alarms + watermark; do **not** use `rabbitmqctl list_alarms` — absent on 4.3.x). Preserve/extract recovery: **[OPERATOR_RABBITMQ_RECOVERY.md](OPERATOR_RABBITMQ_RECOVERY.md)**.
