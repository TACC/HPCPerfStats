...
4. **Compose file:**
   ...
   **Startup day-close discover stall:** if **`sync_timedb`** logs **`startup ingest maintenance waiting discover=True`** with no **`discover: scans elapsed_s=…`** after head metadata (~4 minutes on large archives), check **`{archive_dir}/.sync_timedb_startup_day_close.json`** (`phase`, `pending_retry`, `pending_eligible`, `last_progress`). Do **not** delete the manifest (missing file restarts discover). With current code, discover should finish after janitor snapshot mapping (no 162k-path ready-set on startup) and honor **`pending_eligible`** handoff on restart. If stuck at **`discover_slice_begin`** longer than **`sync_startup_discover_slice_stall_seconds`** (default 900s) with empty **`pending_retry`**, the watchdog forces **`phase=done`** so ingest can start while deferred days remain in **`pending_eligible`**. Inspect async manifest **`.sync_timedb_async_day_close.json`** separately — **`deferred`** rows do not block discover inflight. These startup paths run only when the pipeline command includes **`all`** (see below).

   **Startup maintenance (`all` only):** full startup maintenance (janitor **`reason=startup`** heavy snapshot, startup raw-removal preflight, startup day-close discover, tail ingest, and the startup ingest gate) runs only when **`sync_timedb.py`** is invoked with **`all`** as the date range (production supervisord example: `sync_timedb.py all`). Default date-window runs (no **`all`**) skip startup maintenance and begin ingest immediately; periodic janitor maintenance (`every_n_chunks`, ingest queue drain) still runs.

   **`sync_timedb.py` date arguments:** with no dates, ingest uses the last five calendar days through now. A single **`YYYY-MM-DD`** limits ingest to that day only. Two dates set an explicit start/end range. Prefix **`once`** to exit after one idle rescan (for example **`once 2024-01-15`** or **`once all`**).

   **Startup archive scan (single-flight):** on large trees with **`all`**, janitor startup maintenance and startup prefights share one **`build_archive_maintenance_snapshot`** via **`StartupArchiveScanCoordinator`** — tune **`sync_startup_snapshot_wait_seconds`** (default **300**, min **120**) in **`[PIPELINE]`** if prefights log long waits before **`startup archive scan ready`**. Details: **`docs/DEPLOY_CONCURRENCY_AND_NUMA.md`** § canonical startup archive scan.
