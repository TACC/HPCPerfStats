...
| **`sync_ingest_imap_inflight_cap`** + **`sync_pool_process_cap`** | Cap **concurrent** in-flight ingest tasks (RSS guard for giant-file parse); chunk dispatch uses a **sliding window** so freed worker slots immediately pull the next oldest pending file — not sequential fixed sub-batches |
