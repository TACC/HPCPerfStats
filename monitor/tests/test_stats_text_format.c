...
static void test_snprintf_stats_row_sparse_tiers(void)
{
  ...
  assert(stats_format_snprintf_stats_row_tier(buf, sizeof(buf), &type, s, STATS_ROW_FAST) > 0);
  assert(strcmp(buf, "host_tt dev0 @fast 100 300") == 0);

  assert(stats_format_snprintf_stats_row_tier(buf, sizeof(buf), &type, s, STATS_ROW_FULL) > 0);
...
static void test_sparse_rows(void)
{
  ...
  assert(stats_format_snprintf_stats_row_tier(buf, sizeof(buf), &type, s, STATS_ROW_LEGACY) > 0);
  assert(strcmp(buf, "host_tt dev0 100 200 300 400") == 0);
...
int main(void)
{
  ...
  struct schema_entry *se_control = (struct schema_entry *)(void *)blob_control;
  ...
  {
    char suf[32];
    size_t n = stats_format_schema_entry_suffix(suf, sizeof(suf), se_control);
...

  {
    ...
    struct schema_entry *se_ev = (struct schema_entry *)(void *)blob_ev;
    ...
    char suf[64];
    size_t n = stats_format_schema_entry_suffix(suf, sizeof(suf), se_ev);
