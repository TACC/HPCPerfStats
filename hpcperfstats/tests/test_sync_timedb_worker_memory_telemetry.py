...
def test_classify_archive_append_reap_kind():
  assert wm.classify_archive_append_reap_kind(append_ok=True) == wm.REAP_KEEP
  assert wm.classify_archive_append_reap_kind(append_ok=False) == wm.REAP_FAILURE


def test_archive_stats_files_attaches_worker_pid_on_failure(monkeypatch):
  from hpcperfstats.dbload import sync_timedb as st

  monkeypatch.setattr(st, "_archive_stats_files_body", lambda _info: False)
  monkeypatch.setattr(
      st, "_release_ingest_worker_memory", lambda _p: {"worker_pid": 12345},
  )
  monkeypatch.setattr(st, "record_worker_stage", lambda *_a, **_k: None)
  result = st.archive_stats_files(("/data/day.tar.zst", ["/raw/1"]))
  assert isinstance(result, st.ArchiveAppendOutcome)
  assert result.ok is False
  assert result.worker_pid == 12345


def test_handle_archive_worker_memory_retires_on_append_failure(monkeypatch):
  from hpcperfstats.dbload import sync_timedb as st
  import hpcperfstats.dbload.lib.conf_parser as cfg
  ...
  monkeypatch.setattr(
      st,
      "retire_pool_worker_pid",
      lambda p, pid, context="": retired.append((p, pid, context)) or True,
  )
  st._handle_archive_worker_memory_after_results(
      pool=pool,
      registry=None,
      results=[st.ArchiveAppendOutcome(ok=False, worker_pid=999)],
      deferred_paths=[{
          "task": st.ArchiveTask(
              archive_info=("/day.tar.zst", ["/raw/a"]),
              attempt=1,
          ),
          "paths": ["/raw/a"],
      }],
  ...
  assert retired == [(pool, 999, "archive_failure_reap")]
