...
**Troubleshooting — idle-pool ghost in-flight (multi-hour hang at fixed `remaining`):** Symptom: **`remaining` stuck** (e.g. **102**), no **`loading time`**, ingest workers idle (`futex_wait`/`pipe_read`), main supervisor in **`imap_sliding_window_watch_pool`** → **`time.sleep(poll_timeout_s)`**. Workers show **`pool.worker` → `queues.get()`** (idle, not parsing). **Not** the same as giant-tail supplement (`supplement=yes` fast completions while one giant parses). **Mitigation (code):** reconcile-first — **`INFO: pool imap idle reconcile redispatch round=…`** collects via **`get(timeout=0)`** or redispatches; exit **124** only after reconcile rounds exhausted (`sync_pool_idle_reconcile_max_rounds`, default **3**). **`stats_rows=0`** on completed giants is often **duplicate-skip ingest** — do not use path-tail second alone for `host_data` probes (head timestamp differs). **Operator capture (before restart):** save main + worker py-spy to `/tmp` (`podman exec -u root … py-spy dump --pid $MAINPID`); disk-stat completed giants; grep **`ingest file completed`** for stuck paths. Then **`podman restart hpcperfstats_pipeline_1`** if reconcile does not recover.
