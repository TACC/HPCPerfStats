...
| 2 | `tests/run_redis_cache_pytest_workflow.sh --skip-build` | Fresh compose session, **`test_redis_cache_live.py`** + archive member Redis smoke (`test_archive_members_redis_populate_single_flight_compose`) + **job:v1 Lua** (`test_sync_timedb_job_queue_redis.py`: claim atomicity, reap, catchup isolation, concurrent claim) against real **Redis** (`HPCPERFSTATS_PYTEST_LIVE_REDIS=1`). |
...
| `hpcperfstats/tests/test_sync_timedb_job_queue.py` | Greenfield **job:v1** helpers: atomic Lua claim, short lease TTL + renew, in-flight reaper, path identity vs fingerprint, queue cap / TTL-free keys / `allkeys-*` guard, census (host `FakeRedis`). Drift guard: `test_live_redis_lua_module_covers_loss_and_reap`. Operator census must use prefixed `hpcperfstats:sync_timedb:job:v1:queue:*` keys (`test_operator_job_v1_census_uses_prefixed_queue_keys`). |
| `hpcperfstats/tests/test_sync_timedb_job_queue_redis.py` | Compose live Redis Lua: claim atomicity, lease-conflict non-drop, inflight reap, late-ack fencing, hot/catchup isolation, concurrent claim. Skipped on host; run via `tests/run_redis_cache_pytest_workflow.sh`. |
...
| `hpcperfstats/tests/test_sync_timedb_archive_members_redis_client.py` | Archive-members Redis client timeouts/reset (T1) plus bounded populate wait. |
...
Runs `hpcperfstats/site/lib/machine/tests/test_redis_cache_live.py`, **`hpcperfstats/tests/test_sync_timedb_archive_members_redis.py::test_archive_members_redis_populate_single_flight_compose`**, and **`hpcperfstats/tests/test_sync_timedb_job_queue_redis.py`** inside the `web` container with **`HPCPERFSTATS_PYTEST_LIVE_REDIS=1`**, so Django keeps **`RedisCache`** against the Compose **redis** service instead of switching to `LocMemCache` during pytest. The job-queue module uses Redis DB **15** (`FLUSHDB` per test) so Lua claim/reap cases cannot collide with cache keys on DB 1.
