...
static int intel_skx_imc_parse_pci_path(char *path, uint32_t *bus, uint32_t *dev,
                                        uint32_t *fn)
{
  char *cursor = path;

  if (path == NULL || bus == NULL || dev == NULL || fn == NULL)
    return -1;
  *bus = (uint32_t)strtol(strsep_ne(&cursor, "/"), NULL, 16);
  *dev = (uint32_t)strtol(strsep_ne(&cursor, "."), NULL, 16);
  *fn = (uint32_t)strtol(cursor, NULL, 16);
  return 0;
}
...
static int intel_skx_imc_begin(struct stats_type *type)
{
  ...
  for (i = 0; i < nr_devs; i++) {
    ...
    if (intel_skx_imc_parse_pci_path(dev_paths[i], &bus, &pd, &fn) < 0)
      continue;
    if (intel_skx_imc_begin_dev(bus, pd, fn, mm.map, events, nr_events) == 0)
      nr++;
  }
