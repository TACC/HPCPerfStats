...
|---------|------|----------------|-----------|
| **web** | Gunicorn + Django | Workers = `min(2 * min(os.cpu_count(), effective_cores) + 1, gunicorn_workers)` (default cap **32**); override with **`WEB_CONCURRENCY`** | Each worker may hold a persistent DB connection (`CONN_MAX_AGE`, default **90s** via `conf_parser.get_db_conn_max_age()`) |
...
| **pipeline** | `update_metrics.py` | Titled in-process thread pool size = **`metrics_pool_processes`** | Many concurrent readers during metrics passes |
| **pipeline** | `sync_timedb.py` / archive | Ingest pool = **`get_sync_ingest_pool_processes()`**; archive append pool = **`get_sync_archive_pool_processes()`** — concurrent daily-tar append slots track that pool size (one day per slot). Production coordinator is the **in-process job-store queue orchestrator** (`run_sync_timedb_queue_orchestrator`): exclusive `archive_dir` flock, in-process ingest/append/day_close jobs, sliding-window thread pools. **Cold-path** seal → raw removal → tar-drop runs on **day_close threads** (max **`sync_day_close_max_inflight`**), not a separate ArchiveJanitor tick coordinator | See **Day close (queue orchestrator)** and **Archive zstd priority** below |
| **pipeline** | `listend.py` | **N competing AMQP consume threads** (`listend_amqp_consumer_count`, default **16**, floor **1**, no upper clamp, one pika `BlockingConnection` each) on the original ingest queue; **host-affine archive threads** (**2 × consumer count**, default **32**; leftover `listend_archive_worker_threads` INI ignored) for durable `current` append/rotate + originating-connection threadsafe ack (cheap first-line/`$` host+ts peek, per-host timestamp reorder when N>1, raw AMQP bytes, ack before live-DB submit, sticky flock + kept `current` fd). Drop-mode **`listend_amqp_prefetch`** is **per-consumer** `basic_qos` (default **32**; process-wide unacked = **32×N**; pause keeps prefetch **1** per consumer). Same-redeploy leftover process-wide **512** must be set to **32**. Client+broker **`frame_max=131072`**. Optional live DB dual-write via host-affine **threads** (`listend_db_ingest_*`, default **off**; **32** threads when enabled, **8 GiB** total queue budget **hard-capped at 8** so INI values above 8 are clipped; budget is parse-debt + held archive fds, not 3 MiB×N copied sample strings; **`listend_db_ingest_batch_samples=100`**, **`listend_db_ingest_flush_max_rows=2000`**, **`listend_db_ingest_flush_hold_s=5`**) sharing ordinary `queue.Queue` objects (no `/dev/shm`, no spawn). Flush is bounded by sample count, pending rows, or hold-age so a never-empty queue still writes Timescale. Backpressure when live on: **`listend_db_ingest_backpressure`** default **`drop`** (archive+ack, shed live DB via `queue_drops`) or **`pause`** (high watermark **0.95** / any worker full → stop consume without archive until resume **0.50**; overflow on broker). | Low archive path (parallelized across hosts); DB slots when live ingest on — size with `sync_ingest_pool_processes` against Postgres `max_connections`. When live on, archive-backed catch-up remains completeness repair for live gaps; archive/delete needs a sync_timedb file-complete mark |
| **db** | PostgreSQL | `max_connections=500`; `max_worker_processes=32`, `max_parallel_workers=24`, `max_parallel_workers_per_gather=4`, `max_parallel_maintenance_workers=2`; lowered `work_mem` and maintenance buffers in `docker-compose.yaml` | Hard slot ceiling; per-query memory bounded by GUCs |
...
Ingest is always-on under **`run_sync_timedb_queue_orchestrator`**. Cold-path archive work is **job-store `day_close` jobs** drained by a **`ThreadPoolExecutor`** (max **`sync_day_close_max_inflight`**): age gate → seal → `DayRawRemovalCoordinator.apply_batch_delete` → tar-drop when sealed and no closed raw remains. The B **`ArchiveJanitor`** debt/tick coordinator is **retired** (module deleted).
...
Progress and day_close eligibility are driven by the in-process **job store** plus reconstruct from disk/Timescale (checkpoint `.sync_timedb_state.json` is **not** reconstruct source of truth). day_close jobs refill continuously up to **`sync_day_close_max_inflight`**.
...
| **`rabbitmq` `mem_limit` / `memswap_limit` 96g** plus **`vm_memory_high_watermark.absolute = 80GiB`** | Cgroup hard wall at 96 GiB; publishers block at 80 GiB headroom (`services-conf/rabbitmq_vm_memory.conf`) before Erlang `binary_alloc`. **`ERL_FLAGS=+MBas aobf +MBlmbcs 512 +MHlmbcs 512`** reduces allocator fragmentation. Recreate `rabbitmq` after changing limits, watermark, or **`ERL_FLAGS`**. Lower both mem limits and watermark together on hosts with less than 96 GiB RAM. |
...
| **`sync_ingest_pool_processes`** (default **64**) | Cap live ingest pool width and archive metadata discovery threads; sliding-window **imap inflight equals pool size** (RSS guard for giant-file parse) — not sequential fixed sub-batches |
| **`sync_ingest_pool_maxtasksperchild`** (default **0**) | **Ingest pool only.** Default **`maxtasks=0`**: workers stay alive; supervisor retires only on **failure** / **RSS** (fair-share **`sync_ingest_cooperative_recycle_rss_fraction`**) plus in-worker release; path size alone does **not** retire. Set **`maxtasks=1`** to recycle after every file (drops idle pandas/Django heap). Soak with **`sync_ingest_worker_memory_telemetry=yes`**. **Archive** and **sealed-archive CLI** spawn pools always use hardcoded **`maxtasksperchild=1`** (not this INI). **Idle-pool recover (2026-07-09 / PPID census 2026-07-16):** after full-redispatch thrash, recover uses **abandon-pool** (`abandon_after_kill=True` — SIGKILL `pool._pool` **and** all `[worker:ingest-pool]` children of main, **never** stdlib `Pool.terminate()`/`_help_stuff_finish`) with a recover wall → exit **124** `idle_pool_taskqueue_dead` on timeout; proactive swap same kill scope before recreate; **`reclaim_excess_ingest_pool_children`** keeps `child_ingest ≤ ingest_pool_processes` in-process. |
...
| **Bad `.tar.zst`, good legacy `.tar.gz`** | Remove or rename the corrupt `.tar.zst`; restore sibling `.tar` from gzip (`decompress_compressed_to_tar` / `zstd -d --format=gzip`). Raw stats stay until validation passes. |
 | **Sealed `.tar.zst` unreadable for ingest lookup** | Check `zstd -t /path/YYYY-MM-DD.tar.zst`. If **passes** but tar stream fails (`unexpected end of data`), the tar **payload** is truncated — prefer sibling **mutable `.tar`** when `tar tf` passes (populate auto-fallback; clears stale `archive_day_ingest_skip` when sealed is gone). **`sync_timedb`** and operator scripts may **repair truncated `.tar` in place** (`repair_truncated_daily_tar_in_place`) when members remain extractable; otherwise restore/re-seal from legacy `.tar.gz`. Validate pairs with **`scripts/verify_open_tar_matches_zst.py`**. If **`zstd -t` fails**, the zstd frame is corrupt. When both sealed stream and tar scan fail, ingest logs **`ERROR: daily sealed archive unusable for ingest lookup`** once per day (`reason=zst_frame_invalid` or `tar_truncated_or_unreadable`), sets store **`archive_day_ingest_skip`**, and skips tar-append duplicate-check until repair. **`atomic_seal_tar_to_zst`** refuses seal when tar fails **`verify_tar_archive_readable`**. Active-ingest decompress for append keeps sealed sibling (`remove_compressed=False`). DB-complete ingest continues (`need_archival=False` for skipped days). |
...
Example (hs02 / ini defaults): `32 × 4 + 32 + 32 + 64 + 4 + 2 + 32 = 294` — well under **`max_connections=500`**. A census of hundreds of **idle** backends from `pipeline` is a **leak** (detached `update_metrics` threads after `reset_pool_hard` whose Django sockets were not `connections.close_all()`'d). **Do not raise the GUC** to paper over that; worker `finally` must close thread-local backends because `CONN_MAX_AGE` never fires on abandoned threads.
...
- **`update_metrics` thread workers:** do **not** raise global `db_statement_timeout_ms` for long `host_data` compute. Use **`[PIPELINE] metrics_worker_statement_timeout_ms`** (default **120000**; env **`HPCPERFSTATS_METRICS_WORKER_STATEMENT_TIMEOUT_MS`**; **`0`** disables session timeout during `compute_metrics`). Non-zero values arm metric-row `host__in` chunk-on-timeout splitting; parent no-progress detection remains `metrics_run_stall_timeout_s`. Parent persist uses **`metrics_persist_statement_timeout_ms`**. Simple-metric ORM probes also skip types/events absent from `jid_table.schema` (same idea as SummaryPlot).
- **Staggered supervisord jobs:** [`services-conf/supervisord.conf`](../services-conf/supervisord.conf) starts **`listend`** first (higher priority), then **`sync_timedb`** after **20s**, then **`update_metrics`** after **90s**, so restarts do not open every DB pool at the same instant. The config sets **`stopasgroup=true`** and **`killasgroup=true`** on **`sync_timedb`** so pool workers are torn down when the supervisor exits via **`os._exit(124)`** (reduces orphan **`sync_timedb [main]`** / idle DB sessions after stall). Adjust sleeps, **`priority`**, and kill-as-group for your site. The start **`sleep`** lines are boot stagger only; on stop, supervisord signals programs without those sleeps. **`rsync_data`** runs via **`rsync_data_wrapper.sh`** (prefers `rsync_data.sh`, else `rsync_data.sh.example`); both payloads idle behind a 12h sleep/exit guard until operators enable them.
- **Graceful stop / `stopwaitsecs`:** `sync_timedb` cooperative drain is capped by **`SHUTDOWN_DRAIN_TIMEOUT_S` (120s)**. Unset supervisord **`stopwaitsecs`** defaults to **~10s**, then SIGKILL (and process-group kill for sync_timedb). The tracked config sets **`stopwaitsecs=130`** on **`hpcperfstats-rabbitmq-listener`**, **`sync_timedb`**, and **`update_metrics`**. Compose defaults **`stop_grace_period`** to **30s** for **`web`** / **`pipeline`** (`HPCPERFSTATS_WEB_STOP_GRACE` / **`HPCPERFSTATS_PIPELINE_STOP_GRACE`**); **`scripts/rebuild_pipeline.sh`** stop `-t` defaults are also **30s**. Raise those when you need a full drain before SIGKILL. Solo budgets when grace allows: listend ~**20s**, update_metrics ~**30–60s**, sync_timedb up to **120s**. SIGTERM-driven exit **143** is the T0 expectation in [`OPERATOR_SYNC_TIMEDB_STALL_VERIFY.md`](OPERATOR_SYNC_TIMEDB_STALL_VERIFY.md) when wall clock covers drain.

## Absolute pipeline / portal pool sizes

Pool sizes are absolute INI keys: `sync_ingest_pool_processes` (default **64**), `metrics_pool_processes` (default **32**; in-process metrics + plot/detail prewarm threads), `gunicorn_workers` (default **32**), `listend_db_ingest_pool_processes` (default **32**), `summary_aggregate_prefetch_max_threads` (default **2**). Secondary formula/budget/cap/overlap/`metrics_prewarm_*` gates were removed.
...
**Pool stall guard (exit 124):** When `imap_unordered_watch_pool` or **`imap_sliding_window_watch_pool`** sees **N** consecutive poll timeouts with **no** completed task while all workers are still alive, it raises `MultiprocessingPoolStallError` and `sync_timedb` exits with status **124**. The imap **poll** interval remains **`sync_pool_poll_timeout_s`** (default **5s**); repeated **`WARN: pool imap stall deferred`** lines during legitimate long-ingest or members-store populate waits are **rate-limited** by **`sync_pool_stall_defer_log_interval_s`** (default **60s**; **`0`** = every poll). **Idle-pool ghost in-flight** (workers idle in `futex_wait`/`pipe_read` but `pending_async` non-empty — main thread at **`imap_sliding_window_watch_pool`** poll **`time.sleep`**) is handled **reconcile-first**: **`try_collect_async_result`** (`get(timeout=0)` even when `ready()` is false) collects orphan completions (**H1**); **`reconcile_idle_pending_async`** redispatches stale `AsyncResult` entries after **`sync_pool_idle_reconcile_polls_per_round`** idle polls (default **4**), using ingest idempotency via **`_resolve_streaming_ingest_start`** (not path-tail `host_data` probes). Up to **`sync_pool_idle_reconcile_max_rounds`** redispatch rounds (default **3**) run before **`idle_pool_ghost_inflight`** exit **124** via **`idle_pool_ghost_abort_polls`** (`max(12, min(120, stall_abort_after // 20))` at default **5s** poll). Reconcile also runs during **`maxtasksperchild`** recycle grace when `pending_async` is non-empty (**H2**). **`long_ingest_budget`** defer is **suppressed** when idle ghost is detected (`defer_reason=idle_pool_ghost_inflight`). **Giant-tail / idle-slot supplement** (`supplement=yes` while a **batch-own** path is still in flight) is **healthy** — not ghost stall; supplement prefers **sub-1 GiB** then **[1 GiB, 8 GiB)** from a **supplement_queue** reservoir (`sync_ingest_queue_max_size` × `sync_ingest_giant_pool_supplement_queue_multiplier`, default **3000×2=6000**) at batch start and mid-imap refresh; paths **≥ 8 GiB** stay chunk-only. **RC-E:** once no batch-own path remains in flight, supplement stops (`reason=batch_paths_complete`) — hours of replenish with `in_flight_giants=[]` and no `chunk_elapsed_s` is a bug, not healthy fill. **Per in-flight set**, the legacy dynamic **N** is computed from the largest resolved per-file ingest budget among paths currently dispatched: `int(max_timeout / poll_s) + 1`, clamped between the per-file floor (**900s** → **181** polls at **5s**) and **`sync_pool_stall_abort_after_timeouts`** (**17320** default ceiling = **86600s** wall, **200s above** max per-file). Small-file in-flight sets therefore abort faster than multi‑GiB stragglers. Keep the **ceiling** product **above** `sync_ingest_per_file_timeout_max_s` when raising the per-file max. **`long_ingest_budget`** defer remains a safety net when worker-registry `effective_ingest_timeout_s` exceeds the batch precompute **and workers are not idle**. At startup, when **ceiling** wall ≤ max per-file, the supervisor logs a **WARN**. Per-file budgets are **size-proportional**: `clamp(floor + ceil_mib(size) × per_mib, floor, max)` with defaults mapping **30 GiB → max (24h)**. Per-file **`ingest file`** outcome lines report **`size_bytes`** and **`timeout_s`** (SOP); there is no separate pre-work budget warning. On expiry the file returns `ingest_ok=False` for retry instead of blocking the chunk. Logs include **`batch_max_ingest_timeout_s`**, **`dynamic_stall_abort_after`**, **`dynamic_stall_wall_s`**, plus **`ERROR: Pool imap stalled`** (with optional diagnostics suffix) before exit, and **`WARN: pool imap stall progress`** at 50%/75% of the **current** abort threshold. Stall diagnostics (grep these on the next incident):

**Ingest sliding window:** The queue orchestrator keeps at most `sync_ingest_pool_processes` ingest leases in flight and **refills** on completion (hot vs catchup reserved slots). Do **not** restore B-era sequential sub-batch imap, giant-pool supplement, or dual-mode ``backlog``/``current`` scheduling.


**Dispatch order vs log order:** **`ingest file path=`** lines reflect **pool completion order** within a chunk (parallel workers); they may permute among simultaneous dispatches. **Scheduling order** is in-process job-store ingest ZSET score bands (hot newest-first vs catchup oldest-first) under one orchestrator process. Each chunk logs **`sync_timedb: chunk dispatch begin`** with `paths_sample` and `epochs` before pool dispatch — compare that line to completion lines when investigating skips. **Not** within-chunk noise: **`oldest_day_chunk_gate_fallback`** with **`calendar_days`** showing months ahead of **`oldest_tar`** (cross_day_bucket) while **`pending_n`** is large — fixed by **`oldest_day_chunk_gate_cross_day_defer`** (resume global pending head). **Aligned-only gate:** `oldest_tar` and day-close `unprocessed=` count only paths whose filename/mtime day matches the tar; cross-day misbuckets do not pin early days or inflate waiting_on_ingest. Pending cap merges **all** days’ on-disk unprocessed and may log **`pending cap supplement replace`** when older snapshot paths displace a full newer queue. Grep: `chunk dispatch begin`, `oldest_day_chunk_gate_cross_day_defer`, `pending reconcile cap`, `pending cap supplement replace`.

| Field | Meaning |
|-------|---------|
| `pool_workers_alive` / `in_flight_n` | Alive workers vs imap tasks not yet returned |
| `stall_defer` / `defer_reason` | Why populate defer did or did not reset the stall counter (`store_warm`, `store_populate_active`, `no_day_hint`, **`idle_pool_ghost_inflight`**) |
| `sync_ingest_stall_idle_s` | Soft-requeue when parse/write shows no progress (`-` if unused) |
| `sync_ingest_per_file_timeout_max_s` | Job-store lease EX / external ceiling (not an ingest wall) |
...
| `store_by_day` | Per-day members-store populate snapshot when multiple days in flight |
...
**Cold sealed day vs exit 124:** DB-complete catch-up on one calendar day can block many ingest workers on members-store populate/wait while workers remain alive—this is **not** a dead pool, but imap sees zero completions until populate finishes. Mitigations (code): before each ingest chunk the supervisor **prewarms** member maps once per unique calendar day (`Prewarming archive members for day=…`); **`complete=1` with an empty HASH is not treated as warm** — prewarm re-populates; **DAY_CLOSE seal** invalidates the in-process members store and triggers **supervisor re-prewarm** when the sealed day overlaps an active chunk; while **`in_flight_day_hint`** matches a day with an active populate **lock** (`complete != 1`), the stall counter **defers** even when progress timestamps are stale (`WARN: pool imap stall deferred: members-store populate active for day=…`); ingest workers **suspend idle-clock / SIGALRM guards during members-store populate wait** (`populate_wait_ingest_sigalrm_guard(respect_ingest_deadline=False)`) — populate wait is bounded by **`sync_archive_members_populate_stall_seconds`** / **`sync_archive_members_populate_max_seconds`**. Per-file wall getters (`sync_ingest_per_file_timeout_s`, `sync_pool_stall_abort_after_timeouts`) **must not exist**; raising leftover site INI lines for those names has no effect.

**sync_timedb_archive backfill (sealed-day pool tasks):** `sync_timedb_archive.py` dispatches **one pool task per sealed daily archive** (`.tar.zst` / `.tar.gz`), not per raw stats file. Per-file **`ingest file path=`** lines from the main supervisor therefore do **not** reset the archive backfill stall counter — only whole sealed-day task completion does. The archive tool uses the same stall guard stack as `sync_timedb`: **members-store chunk prewarm** (`INFO: archive chunk prewarm days=…`), **idle-stall / worker-progress defer** (`defer_reason=worker_progress_active` while worker registry stages are fresh), plus members-store populate and long-ingest-budget defer. Poll-count sealed abort helpers (`stall_abort_polls_for_sealed_archives`) **must not exist**; log fields `dynamic_stall_abort_after` / `dynamic_stall_wall_s` stay **0** when present. Context string remains **`sync_timedb_archive pool`** for log grep. If stall ERROR persists with **`worker_stages`** showing stale ages on all workers, inspect DB locks, members-store populate, or a stuck member parse — not merely raise the INI ceiling.
...
**Troubleshooting — no prewarm log + sealed-only day + stall:** If logs show **`ERROR: Pool imap stalled`** with **`in_flight_day_hint`** on a sealed-only day but **no** `Prewarming archive members for day=…`, check (1) members-store sidecar for that day (`lock`, `complete`, `hlen` in stall WARN snapshot), (2) whether DAY_CLOSE **sealed after chunk prewarm** (invalidate without re-prewarm race), (3) silent skip reasons (`Skipping archive members prewarm day=… reason=store_warm` when the complete map was empty). After stall, the process must **`os._exit(124)`** immediately — if `[main]` lingers for hours with defunct children, upgrade to a build with **`_handle_pool_worker_exit_fatal`** and **`SIGKILL`** after pool terminate timeout.
...
**DB-complete ingest + sealed-only days:** When `archive_keep_uncompressed_tar=no`, DB-complete files (`outcome=db_skip`) call `raw_stats_path_needs_tar_append`, which must not stream the full `.tar.zst` once per file **across the ingest pool**. **Populate-pool** workers (`sync_timedb:worker:populate-pool`) run sealed/tar streams; **ingest-pool** and **archive-pool** workers enqueue populate work and wait on the members store. **Populate wait suspends idle-clock / SIGALRM guards** (`respect_ingest_deadline=False`) — wait duration is bounded by populate stall/max limits. Per-file wall timeout names are deleted. **When the members store is fully warm (`complete=1`, non-empty map), duplicate-check uses a single store point lookup per member — not a full-map dump per worker L1 miss.** When both sibling `.tar` and sealed `.tar.zst` exist and the store is warm, duplicate-check must not N× parallel `get_existing_archive_members(tar)` under `file_read_lock`. Per-process L1 plus **members store** coordinate **single-flight populate** per calendar day. The populate winner renews lock/progress on each store batch **and** on a derived heartbeat interval so slow sealed scans do not false-stall. Stall/timeouts raise `ArchiveMembersPopulateStalledError`. When populate cannot read a sealed archive, the winner classifies failure once (`zstd -t` vs tar stream), sets store **`archive_day_ingest_skip`**, and all workers skip tar-append duplicate-check for that day (**no** parallel per-worker point lookups). Ingest does **not** run local sealed scans (see **`sync-timedb-ingest-pool-io-coordination.mdc`**). **`populate_degraded` without day skip** raises `ArchiveMembersStoreUnavailableError` (no local zstd fallthrough). There is **no zstd decompress timeout** on the ingest lookup path; exit **124** is pool-level (zero task completions), not mid-stream zstd abort. `sync_timedb` **hard-fails** mid-ingest duplicate-check on non-transient populate contract failure (`sys.exit(1)` — supervisord restart). Ingest member scans use **plain zstd** (no `nice`/`ionice`); janitor seal/decompress keeps archive priority wrappers.
...
| `sync_ingest_stall_idle_s` | 1800 | Soft-requeue when parse/write shows no progress for this many seconds (`0` disables) |
| `sync_ingest_per_file_timeout_max_s` | 86400 | Job-store lease EX / external ceiling (**24h**); not an ingest wall soft-kill |
| `sync_ingest_giant_pool_supplement_enabled` | yes | Backfill idle pool slots from pending tail while **≥ 2 GiB** giants run |
| `sync_ingest_queue_max_size` | 3000 | No-supplement in-memory pending/process queue (also the ingest chunk size) |
...
| `sync_archive_members_populate_max_seconds` | 7200 | Optional absolute populate/wait cap (`0` = off) |
| `sync_archive_members_populate_pool_processes` | 4 | Dedicated `populate-pool` threads for sealed/tar store streaming (ingest/archive pools enqueue + wait only) |

**Operator bulk members-store invalidate (host, post-crash membership reassessment):** wipe `{archive_dir}/.sync_timedb_archive_members/{day}.json` (never the job-store sidecar). From the Compose checkout, run the host CLI (then restarts `pipeline` so worker L1 is cold):

```bash
# Dry-run one day (no sidecar unlink, no restart)
../.venv/bin/python3 scripts/invalidate_archive_members.py \
  --day YYYY-MM-DD --dry-run --archive-dir /path/from/ini

  # Invalidate one day and restart pipeline (default)
../.venv/bin/python3 scripts/invalidate_archive_members.py \
  --day YYYY-MM-DD --archive-dir /path/from/ini

# All days (requires --yes); --no-restart leaves pipeline L1 warm
# Prefer python3.14+ / project venv if host ``python3`` is older than 3.14.
../.venv/bin/python3 scripts/invalidate_archive_members.py \
  --all --yes --archive-dir /path/from/ini
```
...
**Backlog concurrency margin (2026-08):** internal wall soft-kill getters are **deleted**. Soft-requeue uses **`sync_ingest_stall_idle_s`** (default **1800**) when parse/write heartbeats stop; Postgres **`statement_timeout`** remains the absolute DB ceiling. Coordinator submit-age ingest watchdog is **retired** (do not fall back to it). Apply on the **same redeploy** as the code release (no pre-code INI restart).

For large DB sites with slow duplicate-detection or bulk writes, use **idle stall** (`sync_ingest_stall_idle_s`). Do **not** restore size-proportional wall keys or **`sync_pool_stall_abort_after_timeouts`**. Catch-up mitigations for sealed-only archives: **`archive_keep_uncompressed_tar=yes`**, tune **`sync_ingest_pool_processes`**, and rely on store **single-flight populate** warming the day map (not N parallel `zstd` per worker). Ingest-time DLO quarantine for permanently corrupt raw is separate from exit **124**.
