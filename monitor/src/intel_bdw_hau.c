/*! \file intel_bdw_hau.c
 *  Intel Broadwell HA uncore (intel_x86_uncore_hau_bdw).
 */
...
static const struct intel_uncore_pci_cfg intel_bdw_hau_pci_cfg = {
    .pci_dids = dids,
    .nr_pci_dids = 2,
    .events = events,
...
{
  return intel_uncore_pci_begin(&intel_bdw_hau_pci_cfg, type);
}

static void intel_bdw_hau_collect(struct stats_type *type)
{
  intel_uncore_pci_collect(&intel_bdw_hau_pci_cfg, type);
}
