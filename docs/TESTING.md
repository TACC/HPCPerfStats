...
**One-shot job recalculate:** ``update_metrics.py --jid <JID>`` (also ``--jid=<JID>``) recomputes metrics plus detail/plot artifacts for a single job and exits **0**/**1** without entering the date-range scheduler or post-run sleep. Do not combine ``--jid`` with positional date args. Host unit coverage: ``test_main_jid_*`` / ``test_parse_jid_cli_arg_forms`` in ``hpcperfstats/site/lib/machine/tests/test_update_metrics.py``.
