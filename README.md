...
4. **Compose file:**
   ...
   **Startup day-close discover stall:** if **`sync_timedb`** logs **`startup ingest maintenance waiting discover=True`** with no **`discover: scans elapsed_s=…`** after head metadata (~4 minutes on large archives), check **`{archive_dir}/.sync_timedb_startup_day_close.json`** (`phase`, `pending_retry`, `pending_eligible`, `last_progress`). Do **not** delete the manifest (missing file restarts discover). With current code, discover should finish after janitor snapshot mapping (no 162k-path ready-set on startup) and honor **`pending_eligible`** handoff on restart. If stuck at **`discover_slice_begin`** longer than **`sync_startup_discover_slice_stall_seconds`** (default 900s) with empty **`pending_retry`**, the watchdog forces **`phase=done`** so ingest can start while deferred days remain in **`pending_eligible`**. Inspect async manifest **`.sync_timedb_async_day_close.json`** separately — **`deferred`** rows do not block discover inflight.

   **Startup archive scan (single-flight):** on large trees, janitor startup maintenance and startup prefights share one **`build_archive_maintenance_snapshot`** via **`StartupArchiveScanCoordinator`** — tune **`sync_startup_snapshot_wait_seconds`** (default **300**, min **120**) in **`[PIPELINE]`** if prefights log long waits before **`startup archive scan ready`**. Details: **`docs/DEPLOY_CONCURRENCY_AND_NUMA.md`** § canonical startup archive scan.
