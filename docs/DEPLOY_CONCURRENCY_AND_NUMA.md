...
| **pipeline** | `update_metrics.py` | `multiprocessing.Pool` size = **`get_metrics_pool_process_count()`** (≤ **`metrics_pool_processes`**) | Many concurrent readers during metrics passes |
...
| **pipeline** | `listend.py` | Pika consume on one BlockingConnection; **host-affine archive threads** (`listend_archive_worker_threads`, default **8**) for durable `current` append/rotate + threadsafe ack; drop-mode **`listend_amqp_prefetch`** (default **32**; pause keeps prefetch **1**). Optional live DB dual-write via host-affine **threads** (`listend_db_ingest_*`, default **32** threads, **8 GiB** total queue budget, **`listend_db_ingest_batch_samples=100`**) sharing ordinary `queue.Queue` objects (no `/dev/shm`, no spawn). Compose **`pipeline.shm_size: "1g"`** is leftover spawn IPC for `sync_timedb` / `update_metrics` only. Backpressure when live on: **`listend_db_ingest_backpressure`** default **`drop`** (archive+ack, shed live DB via `queue_drops`) or **`pause`** (high watermark **0.95** / any worker full → stop consume without archive until resume **0.50**; overflow on broker). | Low archive path (parallelized across hosts); DB slots when live ingest on — size with `sync_ingest_pool_processes` against Postgres `max_connections`. When live on, archive-backed catch-up remains completeness repair for live gaps; archive/delete needs a sync_timedb file-complete mark |
...
- **`update_metrics` pool workers:** do **not** raise global `db_statement_timeout_ms` for long `host_data` compute. Use **`[PIPELINE] metrics_worker_statement_timeout_ms`** (default **120000**; env **`HPCPERFSTATS_METRICS_WORKER_STATEMENT_TIMEOUT_MS`**; **`0`** disables session timeout during `compute_metrics`). Non-zero values arm metric-row `host__in` chunk-on-timeout split before the per-job SIGALRM (`metrics_run_per_job_timeout_s` / stall timeout). Parent persist still uses **`metrics_persist_statement_timeout_ms`**. Simple-metric ORM probes also skip types/events absent from `jid_table.schema` (same idea as SummaryPlot).
...
**Metrics pool teardown (2026-08-13):** `update_metrics` stall recovery / `reset_pool_hard` / `close_pool` must use shared `terminate_pool_bounded(..., abandon_after_kill=True)` with the `[worker:metrics-pool]` PPID census mark — **never** stdlib `Pool.terminate()`. CPython `_terminate_pool` can hang forever at `p.join()` when workers inherit a flag-only SIGTERM handler (hs04 production: MainThread wedged ~7.5h, 16 zombies unreaped). Abandon must **stop `_worker_handler` + cancel Finalize before SIGKILL**, then Process.join + retry `/proc` reap (hs04 `/pub` recycle left 24Z when the handler repopulated after kill). Pair `metrics_pool_maxtasksperchild` with `metrics_pool_processes` and pipeline cgroup headroom: higher recycle frequency lowers peak RSS but raises fork cost.
