...
void monitor_options_apply_daemon_conf_kv(const char *key, const char *value_line)
{
  ...
  if (strcmp(key, "user") == 0) {
    monitor_cli_heap_dup_setting(&rmq_user, monitor_cli_lit_rmq_user,
                                 value_line);
    monitor_log_info("%s: Setting RMQ user to %s based on file %s\n",
                     app_name, rmq_user, conf_file_name);
  }
  if (strcmp(key, "password") == 0) {
    monitor_cli_heap_dup_setting(&rmq_password, monitor_cli_lit_rmq_password,
                                 value_line);
  ...
  if (strcmp(key, "jobid_file") == 0) {
    monitor_cli_heap_dup_setting(&jobid_file_path,
                                 monitor_cli_lit_jobid_file_path, value_line);
