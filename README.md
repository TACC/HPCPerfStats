...
| RabbitMQ memory cap | Compose **`mem_limit` / `memswap_limit` 96g** plus `services-conf/rabbitmq_vm_memory.conf` (`vm_memory_high_watermark.absolute = 80GiB` headroom). Erlang allocator: **`ERL_FLAGS=+MBas aobf +MBlmbcs 512 +MHlmbcs 512`**. Logging: console **`warning`** (not **`info`**); **`ERL_CRASH_DUMP_SECONDS=0`**. Inspect: `docker compose exec rabbitmq rabbitmqctl status` (Alarms + watermark; do **not** use `rabbitmqctl list_alarms` — absent on 4.3.x). Recovery: **[docs/OPERATOR_RABBITMQ_RECOVERY.md](docs/OPERATOR_RABBITMQ_RECOVERY.md)**. |
