...
**Idle-pool ghost / exit 124 after full redispatch thrash (T1, 2026-07-08):** during cooperative recycle (`sync_ingest_pool_maxtasksperchild=0`) + fast `db_skip` / `giant_reap`, pre-fix signature was three **`INFO: pool imap idle reconcile redispatch round=… redispatched_n=N pending_async_n=N`** with identical `pending_sample`, workers `futex_wait_queue`, then **`ERROR: … idle_pool_ghost_inflight`** → **`hard exit code=124`** with **`likely_cause=unknown`**. Post-fix (2026-07-09 taskqueue root-cause): after full-redispatch thrash, expect **`INFO: pool imap idle reconcile pool_recover`** then **`pool_recover skip_probe begin`** (`duplicate_pending_n=0` when dedupe OK), **`pool_recover terminate workers_before=… wchan_sample=…`**, **`pool_recover terminate elapsed_s=…`**, **`pool_recover terminate outcome=all_done|sigkill|timeout`**, **`pool_recover respawn dispatch_probe ok`**, **`pool_recover resubmit n=…`**, and **`INFO: pool imap idle reconcile pool_recover done`** with resumed **`ingest file path=`** — **without** a multi-minute gap after **`pool_recover terminate begin`**. Preventive (same deploy): **`WARN: ingest pool replacement lagging`**, **`retire deferred`** near max inflight, proactive swap after failed post-retire **`dispatch_probe`**. **Duplicate-pending signature (hpcperfstats03):** same basename repeated 8× while `pending_async_n=24` → **`duplicate_pending_n=7`** after dedupe (17 unique). Pre-fix showed 24× per-path **`pool_recover skip=no`** lines; post-fix uses one skip summary. **`WARN: pool imap duplicate dispatch suppressed`** means sliding-window dedupe blocked a second in-flight normpath. If recover fails, fatal must include **`likely_cause=idle_pool_taskqueue_dead`**. Optional WARN **`retire skipped missing worker_pid … likely_cause=meta_or_registry_gap`** must stay WARN-only. Distinguish from exit **137** recycle troubleshooting above.

```bash
cd HPCPerfStats
docker compose logs pipeline --since 6h 2>&1 | grep -E 'idle reconcile redispatch|idle reconcile pool_recover|pool_recover skip_probe|pool_recover terminate (workers_before|outcome|elapsed_s)|pool_recover respawn dispatch_probe|pool_recover resubmit|ingest pool replacement lagging|retire deferred|duplicate dispatch suppressed|duplicate_pending_n|idle_pool_ghost_inflight|idle_pool_taskqueue_dead|retire skipped missing worker_pid|hard exit code=124' | tail -80
```

**Expect:** redispatch → `pool_recover` with **`pool_recover done`**, **`terminate outcome=`**, **`dispatch_probe ok`**, and resumed ingest; **no** 8× repeated basename in recover logs; **no** `likely_cause=unknown` on ghost fatals; if 124 remains, `idle_pool_taskqueue_dead` is present.
