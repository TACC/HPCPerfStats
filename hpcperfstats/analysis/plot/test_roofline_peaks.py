...
def test_infer_roofline_peaks_intel_prefers_earlier_imc_generation_in_list():
  ...
  jt = _make_jt({"intel_hsw_imc": [], "intel_skx_imc": []})
  gf, bw = infer_cpu_roofline_peak_flops_and_bw_gbps(jt)
...
def test_infer_roofline_peaks_amd_default_when_amd_counters_present():
  jt = _make_jt({"amd64_pmc": [], "amd64_df": []})
  gf, bw = infer_cpu_roofline_peak_flops_and_bw_gbps(jt)
...
def test_infer_roofline_peaks_grace_when_arm_imc_present():
  jt = _make_jt({"arm_imc": [], "cpu_counter_metrics": []})
  gf, bw = infer_cpu_roofline_peak_flops_and_bw_gbps(jt)
...
def test_infer_cpu_roofline_prefers_roofline_hw_peak_when_complete():
  t0 = pd.Timestamp("2024-06-01 12:00:00+00:00")
  jt = _make_jt(
      {"roofline_hw_peak": [], "intel_skx_imc": []},
      {
...
def test_infer_cpu_roofline_partial_roofline_hw_peak_falls_back_to_legacy():
  t0 = pd.Timestamp("2024-06-01 12:00:00+00:00")
  jt = _make_jt(
      {"roofline_hw_peak": [], "intel_hsw_imc": []},
      {
          ("roofline_hw_peak", "value", ("cpu_peak_fp64_flops_per_s",)): [
              ("n1.cluster", t0, 12_800_000_000_000.0),
          ],
...
def test_infer_gpu_roofline_returns_none_for_partial_roofline_hw_peak():
  t0 = pd.Timestamp("2024-06-01 12:00:00+00:00")
  jt = _make_jt(
      {"roofline_hw_peak": []},
      {
