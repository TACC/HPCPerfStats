...
| `hpcperfstats/tests/test_sync_timedb_archive_members_redis.py` | Redis L2 single-flight populate, incremental HASH + `complete` signal, waiter semantics, `dedupe_hint`, sealed-scan → `populate_archive_members_redis` wiring (`test_populate_redis_members_from_sealed_scan_wires_stream_fn`), hard-fail when Redis required (host `FakeRedis`; compose optional). |
