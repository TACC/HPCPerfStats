...
Same 64 B/CAS conversion is applied by callers before passing frames/scalars here.
Absent or all-non-finite sides are ignored; when both sides are usable they are summed.
"""
...
def agg_sum_val_to_bw_frame(agg: Optional[pd.DataFrame]) -> Optional[pd.DataFrame]:
  """Convert aggregate ``sum_val`` frame to ``bw_gb``; drop when empty or all non-finite."""
