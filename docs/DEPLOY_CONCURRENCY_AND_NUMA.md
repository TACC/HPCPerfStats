...
**Pool stall guard (exit 124):** When `imap_unordered_watch_pool` sees **N** consecutive poll timeouts with **no** completed task while all workers are still alive, it raises `MultiprocessingPoolStallError` and `sync_timedb` exits with status **124**. **Per imap sub-batch**, `N` is computed from the largest resolved per-file ingest budget in that batch: `int(max_timeout / poll_s) + 1`, clamped between the per-file floor (**900s** → **181** polls at **5s**) and **`sync_pool_stall_abort_after_timeouts`** (**2881** default ceiling ≈ **14405s**). Small-file batches therefore abort faster than multi‑GiB stragglers. Keep the **ceiling** product **above** `sync_ingest_per_file_timeout_max_s` when raising the per-file max. **`long_ingest_budget`** defer remains a safety net when worker-registry `effective_ingest_timeout_s` exceeds the batch precompute. At startup, when **ceiling** wall ≤ max per-file, the supervisor logs a **WARN**. Per-file budgets are **size-proportional**: `clamp(floor + ceil_mib(size) × per_mib, floor, max)` with defaults mapping **5 GiB → max**. Workers log **WARN: ingest per-file timeout budget** when resolved budget **≥ 1800s** (30 minutes). On expiry the file returns `ingest_ok=False` for retry instead of blocking the chunk. Logs include **`batch_max_ingest_timeout_s`**, **`dynamic_stall_abort_after`**, **`dynamic_stall_wall_s`**, plus **`ERROR: Pool imap stalled`** (with optional diagnostics suffix) before exit, and **`WARN: pool imap stall progress`** at 50%/75% of the **batch** abort threshold. Stall diagnostics (grep these on the next incident):
...
| `batch_max_ingest_timeout_s` | Max resolved per-file budget for current imap sub-batch |
| `dynamic_stall_abort_after` / `dynamic_stall_wall_s` | Batch-scoped poll abort count and wall seconds |
