...
- **`cpu_counter_metrics` (DCGM + PAPI hybrid on Grace)**: DCGM publishes util accumulators and **`dcgm_cpu_power_util_w`** / **`dcgm_cpu_power_limit_w`** (fields 1130/1131 on **`DCGM_FE_CPU`**; per-socket watts repeated on each logical CPU in that socket). **Fail-soft cycle keys** (always accumulated from DCGM util + `clock_khz` when PAPI cycles are sparse): **`mperf`** = reference cycles (`ref = clock_khz × Δt_us / 1000`), **`aperf`** = active cycles (`act = ref × (util_total / 100)`), **`cpu_clock_est_cycles`** = same as **`aperf`** under fail-soft. Statically linked **PAPI** (pin **7.2.0**) may overwrite those three when measured cycles are nonzero, and fills **`fp_arith_inst_retired_scalar_single`**, **`fp_arith_inst_retired_scalar_double`**, **`arm_est_flops`** (= SP+DP only), and Neoverse **`arm_int8_ops`** / **`arm_int16_ops`** (`ASE_SVE_INT8_SPEC` / `ASE_SVE_INT16_SPEC`). DCGM synthetic FLOPs (and non-cycle synthetic counters) are disabled when PAPI is compiled in.
...
3. For non-x86 targets, native/static builds need `libdcgm` (util/power) plus static **PAPI** from the bundle (`libpapi.a`). Foreign cross smoke often uses `--disable-hardware` when the sysroot lacks DCGM.
4. Export one of:
