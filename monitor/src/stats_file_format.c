/* On-disk stats archive header parsing and mark/schema suffix emission. */
#include <stdarg.h>
#include <stdio.h>
#include <string.h>
...
void stats_file_fprint_mark_multiline(FILE *f, int mark_char, const char *payload)
{
  if (f == NULL)
    return;
  stats_format_emit_mark_multiline(stats_file_fmt_emit, f, mark_char, payload);
}
...
void stats_file_fprint_schema_entry_suffix(FILE *f, struct schema_entry *se)
{
  ...
  if (f == NULL || se == NULL)
    return;
  n = stats_format_schema_entry_suffix(buf, sizeof(buf), se);
  if (n >= sizeof(buf))
    return;
  fputs(buf, f);
}
