...
static void snb_cbo_begin_visit(void *ctx, char *cpu, int pkg_id, int nr_cores)
{
  struct snb_cbo_begin_ctx *c = ctx;
  int j;
...
{
  int n_pmcs = 0;
  struct snb_cbo_begin_ctx ctx = {0};

  if (signature(&n_pmcs) != SANDYBRIDGE)
    goto out;

  intel_topology_foreach_pkg_leader_core(&ctx, snb_cbo_begin_visit);
...
{
  struct snb_cbo_collect_ctx ctx = {type};

  intel_topology_foreach_pkg_leader_core(&ctx, snb_cbo_collect_visit);
}
