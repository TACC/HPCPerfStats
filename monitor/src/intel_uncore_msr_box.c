...
{
  return (uint64_t)((0x0ULL << 0) | (0x00ULL << 10) | (0x1FULL << 18) | (0x000ULL << 23));
}

int intel_uncore_cbo_snb_ivb_begin_box(char *cpu, int box, uint64_t *events, size_t nr_events)
{
  int rc = -1;
  char msr_path[80];
  int msr_fd = -1;
  ...
  if (pwrite(msr_fd, &filter, sizeof(filter), SNB_CBOX_FILTER0 + offset) < 0) {
    ERROR("cannot modify CBo filters: %m\n");
    goto out;
  }

  for (i = 0; i < nr_events; i++) {
    TRACE("MSR %08X, event %016llX\n", SNB_CTL0 + offset + (int)i, (unsigned long long)events[i]);
    if (pwrite(msr_fd, &events[i], sizeof(events[i]), SNB_CTL0 + offset + (int)i) < 0) {
      ERROR("cannot write event %016llX to MSR %08X through `%s': %m\n",
            (unsigned long long)events[i], (unsigned)(SNB_CTL0 + offset + (int)i), msr_path);
      goto out;
    }
  }
...
void intel_uncore_cbo_snb_ivb_collect_box(struct stats_type *type, char *cpu, int pkg_id, int box,
...
{
  return (uint64_t)((0x0ULL << 0) | (0x00ULL << 10) | (0x3FULL << 18) | (0x000ULL << 23));
}

static uint64_t hsw_bdw_cbox_filter1(void)
{
  return (uint64_t)((0x0000ULL << 0) | (0x00ULL << 20) | (0x0ULL << 30) | (0x0ULL << 31));
}

int intel_uncore_cbo_hsw_bdw_begin_box(char *cpu, int box, uint64_t *events, size_t nr_events)
{
  int rc = -1;
  char msr_path[80];
  int msr_fd = -1;
  ...
  if (pwrite(msr_fd, &filter, sizeof(filter), HSW_CBOX_FILTER0_0 + offset) < 0) {
    ERROR("cannot modify CBo Filter 0 : %m\n");
    goto out;
  }
  filter = hsw_bdw_cbox_filter1();
  if (pwrite(msr_fd, &filter, sizeof(filter), HSW_CBOX_FILTER1_0 + offset) < 0) {
    ERROR("cannot modify CBo Filter 1: %m\n");
    goto out;
  }

  for (i = 0; i < nr_events; i++) {
    TRACE("MSR %08X, event %016llX\n", HSW_CTL0 + offset + (int)i, (unsigned long long)events[i]);
    if (pwrite(msr_fd, &events[i], sizeof(events[i]), HSW_CTL0 + offset + (int)i) < 0) {
      ERROR("cannot write event %016llX to MSR %08X through `%s': %m\n",
            (unsigned long long)events[i], (unsigned)(HSW_CTL0 + offset + (int)i), msr_path);
      goto out;
    }
  }
...
void intel_uncore_cbo_hsw_bdw_collect_box(struct stats_type *type, char *cpu, int pkg_id, int box,
                                          const char *const ctr_keys[4])
{
  struct stats *stats = NULL;
  char msr_path[80];
...
int intel_uncore_cha_skx_begin_box(char *cpu, int box, uint64_t *events, size_t nr_events)
{
  int rc = -1;
  char msr_path[80];
  int msr_fd = -1;
  ...
  filter = 0x01e20000;
  if (pwrite(msr_fd, &filter, sizeof(filter), SKX_CHA_FILTER0_0 + offset) < 0) {
    ERROR("cannot modify CHA Filter 0 : %m\n");
    goto out;
  }
  filter = 0x3b;
  if (pwrite(msr_fd, &filter, sizeof(filter), SKX_CHA_FILTER1_0 + offset) < 0) {
    ERROR("cannot modify CHA Filter 1: %m\n");
    goto out;
  }

  for (i = 0; i < nr_events; i++) {
    TRACE("MSR %08X, event %016llX\n", SKX_CTL0 + offset + (int)i, (unsigned long long)events[i]);
    if (pwrite(msr_fd, &events[i], sizeof(events[i]), SKX_CTL0 + offset + (int)i) < 0) {
      ERROR("cannot write event %016llX to MSR %08X through `%s': %m\n",
            (unsigned long long)events[i], (unsigned)(SKX_CTL0 + offset + (int)i), msr_path);
      goto out;
    }
  }
...
void intel_uncore_cha_skx_collect_box(struct stats_type *type, char *cpu, int pkg_id, int box,
                                      const char *const ctr_keys[4])
{
  struct stats *stats = NULL;
  char msr_path[80];
