...
Long-running ingest and metrics jobs should not retry or degrade when the
server is down, still starting, or otherwise unreachable: supervisor restarts
...
class DatabaseUnavailableExit(BaseException):
  """Raised when the database cannot be used; exit the process (supervisor restart)."""
...
def log_and_raise_database_unavailable(exc: BaseException, *, context: str) -> None:
  """Log once and raise :class:`DatabaseUnavailableExit` (non-``Exception`` subtree)."""
  log_print(
...
def reraise_database_unavailable_chain(exc: BaseException, *, context: str) -> None:
  """If ``exc`` or its causes/contexts indicate DB unavailability, terminate the process."""
  seen: set[int] = set()
  cur: BaseException | None = exc
  while cur is not None and id(cur) not in seen:
    seen.add(id(cur))
