...
| `hpcperfstats/tests/test_sync_timedb_job_queue.py` | In-process job-store reconstruct/discover/streaming helpers plus leftover host claim tests. Operator census must use `.sync_timedb_job_store.json` and `[thread:populate-pool]` (`test_operator_census_uses_disk_sidecars_and_thread_titles`). |
| `hpcperfstats/tests/test_sync_timedb_job_queue_redis.py` | Retired live Redis Lua module (skipped unless leftover file is still collected). Django cache live Redis remains `test_redis_cache_live.py`. |
