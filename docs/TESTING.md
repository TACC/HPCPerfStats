...
| `hpcperfstats/tests/test_sync_timedb_startup_day_close.py` | Startup checkpoint-driven async `DAY_CLOSE`: subtract `.sync_timedb_state.json`, submit eligible days to `AsyncDayCloseCoordinator`, candidate report (host unit). Pair with `test_supervisor_starts_ingest_without_waiting_for_startup_day_close` in `test_sync_timedb_supervisor.py`. Quick run: `cd HPCPerfStats && ../.venv/bin/python3 -m pytest -q hpcperfstats/tests/test_sync_timedb_startup_day_close.py hpcperfstats/tests/test_sync_timedb_archive.py -k "checkpoint or day_close_candidate or immediate_day_close_uses_checkpoint"`. |
