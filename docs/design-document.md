...
| **rabbitmq** | Broker for monitor→site message delivery (`rabbitmq:4.3.4-alpine`, AMQP port **5672** only — no management plugin). Admin Monitor queue depth uses the same AMQP path (pika), not HTTP `:15672`. |
